...
export const GridironVisualizer: React.FC<VisualizerProps> = ({
  ...
  useEffect(() => {
    ...
    const isOutside =
      pType.includes("outside") ||
      pType.includes("sweep") ||
      pType.includes("pitch") ||
      pType.includes("toss");
    const isInside =
      pType.includes("inside") ||
      pType.includes("dive") ||
      pType.includes("draw");
    const isGoalKick =
      pType.includes("xp") ||
      pType.includes("field goal") ||
      pType.includes("fg");
    ...
    if (isGoalKick) {
      ...
      leftKeyframes = [
        `${mapToField(ballX)}%`,
        `${(mapToField(ballX) + targetVisualX) / 2}%`,
        `${targetVisualX}%`,
      ];
    ...
    // 2. Air Physics (Punts & Kickoffs)
      ...
      leftKeyframes = [
        `${mapToField(ballX)}%`,
        `${mapToField(ballX + (endX - ballX) * 0.5)}%`,
        `${mapToField(endX)}%`,
      ];
      scaleKeyframes = [1, 3, 1];
    ...
    // 3. Air Physics (Passes & Turnovers)
      ...
      isPass ||
      resultCategory === "INCOMPLETE" ||
      resultCategory === "INTERCEPTION"
    ) {
      if (
        resultCategory === "INCOMPLETE" ||
        resultCategory === "INTERCEPTION"
    ...
    <div className="relative w-full aspect-[2/1] bg-green-700 border-x-8 border-white shadow-2xl rounded-sm overflow-hidden flex items-center">
      ...
      <div className="absolute inset-y-0 left-[10%] right-[10%] flex justify-between">
        {[...Array(11)].map((_, i) => {
          ...
          return (
            <div
              key={i}
              className="h-full flex flex-col justify-between items-center relative"
            >
              <div
                className={`${isGoalLine ? "w-[6px] bg-white" : "w-px bg-white/40"} h-full`}
              />
              {!isGoalLine && (
      ...
      >
        ...
        >
          {badgeConfig && (
            <div
              className={`px-3 py-1 rounded shadow-xl text-white font-black text-[11px] tracking-[0.2em] uppercase border-2 ${badgeConfig.bg} ${badgeConfig.border}`}
            >
              {badgeConfig.text}
            </div>
          )}
