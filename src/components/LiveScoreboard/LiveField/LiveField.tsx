  ...
  ({ title, games, color, onSelect, broadcastState }: any) => (
    <div className="flex flex-col h-full min-h-0 py-2">
      <h3 className={`text-[1.8vh] font-black text-white mb-4 uppercase tracking-[0.2em] border-l-4 ${color} pl-3 text-left shrink-0`}>
        {title} <span className="text-(--text-muted) text-[1.4vh]">({games.length})</span>
      </h3>
      <div className="flex-1 min-h-0 overflow-y-auto pr-2 space-y-3 custom-scrollbar pb-4">
        {games.map((game: any) => (
          ...
          >
            <div className="text-[1.4vh] font-black text-(--text-primary)">{game.AwayTeam} @ {game.HomeTeam}</div>
            {(game.GameComplete || game.IsRevealed) && (
                <div className="text-[1.6vh] font-mono font-bold text-(--accent-success)">{game.AwayTeamScore} - {game.HomeTeamScore}</div>
            )}
          </button>
        ))}
      </div>
...
const LiveField = () => {
  ...
  const { liveGames: firebaseGames } = useLiveFieldState(selectedLeague as League);
  ...
  const rawSeasonID = useMemo(() => {
    return selectedLeague === SimCFB ? chlTs?.CollegeSeasonID : chlTs?.NFLSeasonID;
  ...
  useEffect(() => {
    const fetchAndStitch = async () => {
        ...
        let apiResults: any[] = [];
        try {
            const res = await fetch(`${fbaUrl}games/${endpoint}/season/${rawSeasonID}`);
    ...
    const interval = setInterval(() => {
      setGames((prev) => {
        ...
        Object.values(newGames).forEach((g: any) => {
          ...
          if (plays.length === 0) { g.GameComplete = true; g.IsRevealed = true; return; }
      ...
      <div className="h-screen w-full bg-(--bg-primary) pt-[calc(8vh+10px)] flex flex-col overflow-hidden text-left p-8">
        {isModerator && (
          <div className="bg-(--bg-secondary) border border-(--border-primary) p-4 mb-6 flex justify-between items-center rounded">
            <span className="text-white font-bold uppercase tracking-widest text-sm">Control Room</span>
            <button onClick={triggerEngine} className="bg-green-600 text-white font-bold py-2 px-6 rounded uppercase text-xs">
              {broadcastState === "IDLE" ? "🟢 START BROADCAST" : broadcastState}
            </button>
          </div>
        )}
        <div className="flex gap-4">
            <ButtonGrid>
                <PillButton isSelected={selectedLeague === SimCFB} onClick={() => setSelectedLeague(SimCFB)}>CFB</PillButton>
                <PillButton isSelected={selectedLeague === SimNFL} onClick={() => setSelectedLeague(SimNFL)}>NFL</PillButton>
            </ButtonGrid>
        </div>
        <div className="grid grid-cols-12 gap-4 h-full mt-4">
             <div className="col-span-2 border-r border-white/10">
                <GameMiniList title="Upcoming" games={Object.values(games).filter(g => !g.GameComplete && !g.IsRevealed)} color="border-blue-500" onSelect={setSelectedGameId} broadcastState={broadcastState} />
             </div>
             <div className="col-span-8 overflow-y-auto">
                <div className="grid grid-cols-2 gap-6">
                    {Object.values(games).filter(g => !g.GameComplete && !g.IsRevealed && g.Period > 0).map(g => (
                        <div key={g.GameID} onClick={() => setSelectedGameId(g.GameID)} className="bg-(--bg-secondary) p-6 border border-white/10 cursor-pointer">
                            <div className="text-white font-black text-2xl">{g.AwayTeamScore} - {g.HomeTeamScore}</div>
                        </div>
                    ))}
                </div>
             </div>
             <div className="col-span-2 border-l border-white/10">
                <GameMiniList title="Results" games={Object.values(games).filter(g => g.GameComplete || g.IsRevealed)} color="border-green-500" onSelect={setSelectedGameId} broadcastState={broadcastState} />
             </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="h-screen w-full bg-(--bg-primary) pt-[calc(8vh+10px)] flex flex-col p-8 overflow-hidden">
        <button onClick={() => setSelectedGameId(null)} className="text-(--text-muted) uppercase font-bold mb-4">← BACK</button>
        <div className="grid grid-cols-12 gap-6 h-full">
            <div className="col-span-10">
                <div className="bg-(--bg-secondary) border border-white/10 p-6 flex justify-between items-center text-white">
                    ...
                    <span className="text-5xl font-black">{activeGame.AwayTeamScore} - {activeGame.HomeTeamScore}</span>
                    <h2>{activeGame.HomeTeam}</h2>
                </div>
                <GridironVisualizer ballX={activeGame.Zone} playType="IDLE" homeName={activeGame.HomeTeam} awayName={activeGame.AwayTeam} />
                <div className="p-4 bg-black/20 mt-4 h-64 overflow-y-auto">
                    {(currentPlaysRef.current[activeGame.GameID] || []).map((p: any, i: number) => (
                        <div key={i} className="text-white text-xs py-1 border-b border-white/5">{p.PlayText}</div>
                    ))}
                </div>
            </div>
            <div className="col-span-2"><TeamStatsSidebar title="Stats" teamName={activeGame.HomeTeam} stats={null} /></div>
        </div>
    </div>
  );
};
