...
const ThreadPortal: FC<ThreadPortalProps> = ({
  thread,
  backgroundColor,
  headerColor,
  borderColor,
  ...
  return (
    ...
    >
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <Text
          variant="body-small"
          classes={`font-semibold truncate text-start ${textColorClass}`}
        >
          {thread.title}
        </Text>
...
export const ForumPortal = ({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const [pinned, { threads }] = await Promise.all([
          ForumService.GetPinnedThreads(forumId),
          ForumService.GetThreadsByForum(forumId, 5),
        ]);
        if (cancelled) return;
        const combined = [...pinned, ...threads]
          .sort(
            (a, b) =>
              (b.latestActivityAt?.seconds ?? 0) -
              (a.latestActivityAt?.seconds ?? 0),
          )
          .slice(0, 5);
        setLocalThreads(combined);
      } catch {
        if (!cancelled) setLocalThreads([]);
      } finally {
  ...
  return (
    <SectionCards
      ...
      textColorClass={textColorClass}
      darkerBackgroundColor={darkerBackgroundColor}
      onHeaderClick={() => navigate(routes.FORUMS)}
    >
      {localLoading && (
        <Text variant="small" classes={`${textColorClass} pt-2`}>
          Loading threads...
      ...
      {!localLoading && localThreads.length === 0 && (
        <Text variant="small" classes={`${textColorClass} pt-2`}>
          No active threads for this league. Check back later!
        </Text>
      )}
    </SectionCards>
  );
