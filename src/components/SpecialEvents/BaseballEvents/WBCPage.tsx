...
import {
  WBCTeam, WBCRosterPlayer, WBCRostersResponse,
...
const PoolStandingsTable = ({ teams, poolGroup }: { teams: WBCTeam[]; poolGroup: string }) => {
  const sorted = useMemo(() =>
    [...teams].filter((t) => t.pool_group === poolGroup).sort((a, b) => b.pool_wins - a.pool_wins || a.pool_losses - b.pool_losses),
  [teams, poolGroup]);
      ...
      <table className="w-full border-collapse text-sm">
        ...
        <tbody>
          {sorted.map((t, idx) => (
            ...
            >
              <td className="px-2 py-1 text-center text-gray-400 text-xs">{t.seed}</td>
              <td className="px-2 py-1 font-medium">
                <span className="mr-1">{t.country_code}</span>
                <span className="text-xs text-gray-400">{t.country_name}</span>
              </td>
              <td className="px-2 py-1 text-center font-semibold">{t.pool_wins}</td>
              <td className="px-2 py-1 text-center">{t.pool_losses}</td>
            </tr>
          ))}
          {sorted.length === 0 && (
            <tr><td colSpan={4} className="px-2 py-4 text-center text-gray-400 text-xs">No teams in this pool.</td></tr>
          )}
        </tbody>
      </table>
    </div>
  ...
  return (
    <div>
      ...
      <table className="w-full border-collapse text-sm">
        ...
        <tbody>
          {players.map((p, idx) => (
            <tr key={p.player_id} className={`border-b border-gray-100 dark:border-gray-700 ${idx % 2 === 0 ? "bg-gray-50/50 dark:bg-gray-800/30" : ""}`}>
              <td className="px-2 py-1 text-center text-gray-400">{idx + 1}</td>
              <td className="px-2 py-1 font-medium">{p.name}</td>
              <td className="px-2 py-1 text-center uppercase text-xs">{p.position}</td>
            </tr>
          ))}
          {players.length === 0 && (
            <tr><td colSpan={3} className="px-2 py-4 text-center text-gray-400 text-xs">No roster data.</td></tr>
          )}
        </tbody>
      </table>
    </div>
...
const KnockoutBracket = ({ bracket }: { bracket: PlayoffBracketResponse | null }) => {
  if (!bracket || Object.keys(bracket.rounds).length === 0) {
    return (
      <div className="py-8 text-center">
        <Text variant="body" classes="text-gray-400">Knockout bracket not yet available.</Text>
      </div>
    );
  }
...
export const WBCPage = () => {
  ...
  const { seasonContext, mlbOrganization, bootstrappedOrgId, loadBootstrapForOrg, specialEvents } = useSimBaseballStore();
  ...
  const [knockoutBracket, setKnockoutBracket] = useState<PlayoffBracketResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const evts = await BaseballService.GetSpecialEvents(seasonContext.current_league_year_id);
        const wbcEvt = evts.events.find((e) => e.event_type === "wbc" && e.id != null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const data = await BaseballService.GetPlayoffBracket(seasonContext.current_league_year_id, 99);
    ...
    <PageContainer>
      <div className="flex-col w-[95vw] sm:w-[90vw] md:w-full md:mb-6 px-2">
        ...
        >
          ...
          <div>
            <Text variant="h4" classes={headerTextClass}>World Baseball Classic</Text>
        ...
        <TabGroup>
          {(["Pools", "Rosters", "Knockout"] as WBCTab[]).map((tab) => (
            <Tab key={tab} label={tab} selected={activeTab === tab} setSelected={(val) => setActiveTab(val as WBCTab)} />
          ))}
        </TabGroup>
        ...
        <Border classes="p-4" styles={{ borderTop: `3px solid ${headerColor}` }}>
          {!wbcEvent ? (
            <div className="flex items-center justify-center py-12">
              <Text variant="body" classes="text-gray-400">No World Baseball Classic event found for this season.</Text>
            </div>
          ) : (
            <>
              {/* Pools Tab */}
              {activeTab === "Pools" && (
                <div className="flex gap-6 flex-wrap">
                  {poolGroups.map((group) => (
                    <PoolStandingsTable key={group} teams={teams} poolGroup={group} />
                  ))}
                  {poolGroups.length === 0 && (
                    <Text variant="body" classes="text-gray-400">No pool data available.</Text>
                  )}
                </div>
              )}
                ...
                <div>
                  {rostersLoading ? (
                    <div className="flex items-center justify-center py-8">
                      <Text variant="body" classes="text-gray-500 dark:text-gray-400">Loading rosters...</Text>
                    </div>
                  ) : (
                    <>
                      ...
                      {selectedCountry && rosters?.rosters[selectedCountry] && (
                        <CountryRosterTable countryCode={selectedCountry} players={rosters.rosters[selectedCountry]} />
                      )}
                      {countryList.length === 0 && (
                        <Text variant="body" classes="text-gray-400">No roster data available.</Text>
                      )}
                    </>
                  )}
                </div>
