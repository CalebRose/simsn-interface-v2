        ...
        <tbody>
          {sorted.map((p, idx) => (
            ...
            >
              ...
              <td className="px-2 py-1.5 text-center uppercase">{p.position}</td>
              <td className="px-2 py-1.5 text-center text-xs">{p.team}</td>
              <td className="px-2 py-1.5 text-center">
                {p.is_starter ? (
                  <span className="text-xs bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 px-2 py-0.5 rounded-full">Starter</span>
                ) : (
                  <span className="text-xs text-gray-400">Reserve</span>
                )}
              </td>
              <td className="px-2 py-1.5 text-center">
                <span className={`text-xs ${p.source === "auto" ? "text-green-600 dark:text-green-400" : "text-purple-600 dark:text-purple-400"}`}>
          ...
          {players.length === 0 && (
            <tr>
              <td colSpan={6} className="px-4 py-8 text-center text-gray-400">No roster data available.</td>
            </tr>
          )}
        </tbody>
      </table>
...
const ScoreDisplay = ({ gameResult, status }: { gameResult: AllStarGameResult | null; status?: string }) => {
  if (!gameResult) {
    return (
      <div className="text-center py-6">
        <Text variant="body" classes="text-gray-400">Game has not been played yet.</Text>
        {status && (
          <Text variant="small" classes="text-gray-400 mt-1">
            Status: <span className="capitalize font-medium">{status.replace("_", " ")}</span>
          </Text>
        )}
      </div>
    );
  ...
  const alWon = gameResult.game_outcome === "AWAY_WIN" || (gameResult.away_score > gameResult.home_score);
  const nlWon = !alWon;

  return (
    <div className="flex items-center justify-center gap-8 py-6">
      <div className={`text-center ${alWon ? "font-bold" : "opacity-60"}`}>
        <Text variant="h4" classes={alWon ? "" : "text-gray-400"}>AL</Text>
        <Text variant="h4">{gameResult.away_score}</Text>
      </div>
      <div className="text-gray-400 text-sm">vs</div>
      <div className={`text-center ${nlWon ? "font-bold" : "opacity-60"}`}>
        <Text variant="h4" classes={nlWon ? "" : "text-gray-400"}>NL</Text>
    ...
    <PageContainer>
      <div className="flex-col w-[95vw] sm:w-[90vw] md:w-full md:mb-6 px-2">
        ...
        >
          ...
          <div>
            <Text variant="h4" classes={headerTextClass}>All-Star Game</Text>
        ...
        <Border classes="p-4" styles={{ borderTop: `3px solid ${headerColor}` }}>
          {!allStarEvent ? (
            <div className="flex items-center justify-center py-12">
              <Text variant="body" classes="text-gray-400">No All-Star Game event found for this season.</Text>
            </div>
          ) : (
            <>
              {/* Score */}
              <ScoreDisplay gameResult={allStarEvent.game_result ?? null} status={allStarEvent.status} />

              {/* Roster Tabs */}
              <TabGroup>
                <Tab label="American League" selected={activeTab === "AL"} setSelected={() => setActiveTab("AL")} />
                <Tab label="National League" selected={activeTab === "NL"} setSelected={() => setActiveTab("NL")} />
              </TabGroup>
