            ...
            <div className="flex flex-col lg:grid lg:grid-cols-[2fr_10fr] gap-4 w-full h-full">
                <div className="w-full">
                    {nflTeam && nflTeam.Capsheet ? (
                        ...
                        <div className="w-full p-6 border-2 rounded-xl flex flex-col items-center justify-center shadow-lg" style={{ borderColor: teamColors.One, backgroundColor: 'rgba(0,0,0,0.4)', minHeight: '250px' }}>
                            <Text variant="h6" classes="text-white font-bold uppercase tracking-widest">Team Capsheet</Text>
                            <Text variant="small" classes="text-gray-400 animate-pulse mt-2">Loading financial data...</Text>
                        </div>
                    )}
                </div>

                <div className="flex flex-col w-full gap-y-4">
                    <div className="w-full p-4 flex flex-row items-center justify-between border-2 rounded-xl" style={{ borderColor: teamColors.One, backgroundColor: 'rgba(0,0,0,0.4)' }}>
                        <ButtonGroup>
                    ...
                    {viewCategory === Overview && (
                        <div className="w-full p-4 flex flex-row flex-wrap gap-4 border-2 rounded-xl items-end" style={{ borderColor: teamColors.One, backgroundColor: 'rgba(0,0,0,0.4)' }}>
                            <CategoryDropdown label="Positions" options={FootballPositionOptions} isMulti={true} change={(opts: any) => { setPositions(opts ? opts.map((o: any) => o.value) : []); setCurrentPage(0); }} />
                            <CategoryDropdown label="Archetype" options={FootballArchetypeOptions} isMulti={true} change={(opts: any) => { setArchetypes(opts ? opts.map((o: any) => o.value) : []); setCurrentPage(0); }} />
                        </div>
                    )}

                    <div className="w-full p-4 border-2 rounded-xl shadow-2xl overflow-hidden min-h-[400px]" style={{ borderColor: teamColors.One, backgroundColor: 'rgba(0,0,0,0.4)' }}>
                        {viewCategory === Overview ? (
                            <>
                                <Table
                                    team={(nflTeam || {}) as any}
                                    ...
                                    rowRenderer={(player: any, index: number, bg: string) => {
                                        ...
                                        return (
                                            <div className="table-row border-b border-gray-700 last:border-0" style={{ backgroundColor: bg }} key={player.ID}>
                                                ...
                                                <TableCell>
                                                    <button onClick={(e) => { e.stopPropagation(); addPlayerToUDFABoard(player); }} disabled={isOnBoard} className="p-2 rounded-lg flex items-center justify-center bg-gray-700 hover:scale-110 shadow-lg">
                            ...
                            <Table
                                ...
                                rowRenderer={(profile: any, index: number, bg: string) => (
                                    <div className="table-row border-b border-gray-700 last:border-0" style={{ backgroundColor: bg }} key={profile.ID}>
                                        ...
                                        <TableCell>
                                            <input type="number" value={profile.Points} min={1} max={20} onChange={(e: any) => handlePointChange(profile.ID, parseInt(e.target.value) || 0)} className="w-24 bg-gray-900 text-white border border-gray-600 rounded p-1 text-center font-bold text-lg" />
                                        </TableCell>
