...
export const ExtensionOfferModal: FC<ExtensionOfferModalProps> = ({
  ...
  >(() => {
    if (league === SimNFL) {
      return new NFLExtensionOffer({ existingOffer });
    }
    if (league === SimNBA) {
      return new NBAExtensionOffer({ existingOffer });
    }
    return new PHLExtensionOffer();
  });
  ...
  const isNBA = useMemo(() => {
    return league === SimNBA && offer instanceof NBAExtensionOffer;
  }, [league]);

  const isPHL = useMemo(() => {
    return league === SimPHL && offer instanceof PHLExtensionOffer;
  }, [league]);
  ...
  const totalSalary = useMemo(() => {
    ...
    if (isNBA && offer instanceof NBAExtensionOffer) {
      return (
        offer.Year1Total +
        offer.Year2Total +
        offer.Year3Total +
  ...
  const contractValue = useMemo(() => {
    ...
    if (isNFL && player.Age) {
      return GetNFLContractValue(player.Age, offer as NFLExtensionOffer);
    }
    return 0;
  }, [offer, player]);
  ...
  const ChangeInput = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setOffer((prev) => {
        if (prev instanceof NFLExtensionOffer) {
          let updated = new NFLExtensionOffer({ ...prev }); // Shallow clone with current values
        ...
        if (prev instanceof NBAExtensionOffer) {
          return prev.updateField(name, val);
        }
        if (prev instanceof PHLExtensionOffer) {
          return prev.updateField(name, val);
        }
        return prev;
      });
    ...
    <>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 lg:grid-cols-6 space-y-1 lg:space-y-0 space-x-2 mb-2">
          <div className="flex">
            {(offer instanceof PHLExtensionOffer ||
              offer instanceof NFLExtensionOffer) && (
              <Input
                type="number"
                label="Length"
                name="ContractLength"
        ...
        {isNBA && offer instanceof NBAExtensionOffer && (
