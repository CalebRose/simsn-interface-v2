...
export const RecruitingAdminPanel: FC<RecruitingAdminPanelProps> = ({
  leagueYearId,
}) => {
  const { enqueueSnackbar } = useSnackbar();

  // State
  const [recruitState, setRecruitState] = useState<RecruitingState | null>(
    null,
  );
  const [isLoading, setIsLoading] = useState(true);
  const [isBusy, setIsBusy] = useState(false);
  const [lastAdvance, setLastAdvance] =
    useState<AdvanceRecruitingWeekResponse | null>(null);

  // Reports
  const [reportTab, setReportTab] = useState<ReportTab>("summary");
  const [summary, setSummary] = useState<RecruitingSummaryResponse | null>(
    null,
  );
  ...
  const [demandStarFilter, setDemandStarFilter] = useState<number | undefined>(
    undefined,
  );
  ...
  const loadState = useCallback(async () => {
    try {
      ...
      enqueueSnackbar(e?.message || "Failed to load recruiting state", {
        variant: "error",
      });
    }
    setIsLoading(false);
  ...
  const handleAdvance = useCallback(async () => {
    ...
    setLastAdvance(null);
    try {
      const res = await BaseballService.AdvanceRecruitingWeek({
        league_year_id: leagueYearId,
      });
      setLastAdvance(res);
      setRecruitState((prev) =>
        prev
          ? { ...prev, current_week: res.new_week, status: res.status as any }
          : prev,
      );
      enqueueSnackbar(
      ...
      enqueueSnackbar(e?.message || "Failed to advance recruiting", {
        variant: "error",
      });
    }
    setIsBusy(false);
    ...
    async (tab: ReportTab) => {
      ...
      try {
        switch (tab) {
          ...
          case "org-detail": {
            if (selectedOrgId) {
              const od = await BaseballService.GetOrgDetail(
                selectedOrgId,
                leagueYearId,
              );
              setOrgDetail(od);
        ...
        enqueueSnackbar(e?.message || "Failed to load report", {
          variant: "error",
        });
      }
    ...
    <Border classes="w-full mt-4">
      <div className="p-4">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-3">
          <span
            className={`px-3 py-1 rounded-full text-xs font-semibold ${STATUS_COLORS[recruitState.status]}`}
          >
            {recruitState.status.charAt(0).toUpperCase() +
              recruitState.status.slice(1)}
          </span>
        ...
        {lastAdvance && (
          <div className="border rounded-sm p-3 mb-4 dark:border-gray-700 bg-green-50 dark:bg-green-900/20">
            ...
            {lastAdvance.ranked_players != null && (
              <p className="text-sm text-green-700 dark:text-green-300">
                {lastAdvance.ranked_players.toLocaleString()} players ranked and
            ...
            {lastAdvance.commitments && lastAdvance.commitments.length > 0 && (
              <div className="mt-1">
                <p className="text-xs text-gray-500 mb-1">
                  {lastAdvance.commitments.length} commitment
                  {lastAdvance.commitments.length !== 1 ? "s" : ""}:
                </p>
                <ul className="text-sm space-y-0.5">
                  {lastAdvance.commitments.map((c) => (
                    ...
                    >
                      {c.player_name} ({STAR_DISPLAY(c.star_rating)}) &rarr;{" "}
                      {c.org_abbrev} ({c.points_total} pts)
                    </li>
                  ))}
                </ul>
              </div>
            )}
            {lastAdvance.cleanup_commitments &&
              lastAdvance.cleanup_commitments.length > 0 && (
                <div className="mt-2">
                  ...
                  <ul className="text-sm space-y-0.5">
                    {lastAdvance.cleanup_commitments.map((c) => (
                      <li
                        key={c.player_id}
                        className="text-yellow-700 dark:text-yellow-300 italic"
                      >
                        {c.player_name} ({STAR_DISPLAY(c.star_rating)}) &rarr;{" "}
                        {c.org_abbrev} ({c.points_total} pts)
                      </li>
        ...
        {/* Reports section */}
        <div className="border rounded-sm p-3 mb-3 dark:border-gray-700">
          <Text variant="body-small" classes="font-semibold mb-2">
            Reports
          </Text>
          <div className="flex flex-wrap gap-1 mb-3">
            {(["summary", "leaderboard", "demand"] as ReportTab[]).map(
              (tab) => (
                <button
                  key={tab}
                  ...
                  {summary.stars && (
                    <div className="mb-2">
                      <p className="text-xs text-gray-500 mb-1 font-semibold">
                        Star Distribution (committed / total)
                      </p>
                      <div className="space-y-1">
                        {[5, 4, 3, 2, 1].map((star) => {
                          const total =
                            summary.stars?.star_distribution?.[String(star)] ??
                            0;
                          const committed =
                            summary.stars?.committed_by_star?.[String(star)] ??
                            0;
                          ...
                          return (
                            ...
                            >
                              <span className="w-16 text-yellow-500 font-mono">
                                {STAR_DISPLAY(star)}
                              </span>
                              <div className="flex-1 bg-gray-200 dark:bg-gray-700 rounded-sm h-2 overflow-hidden">
                                <div
                    ...
                    <tbody>
                      {leaderboard.map((org, idx) => (
                        ...
                        >
                          ...
                          <td className="px-2 py-1 text-center">
                            {org.avg_star.toFixed(1)}
                          </td>
                          <td className="px-2 py-1 text-right">
                            {org.total_points_invested.toLocaleString()}
                          </td>
                          <td className="px-2 py-1 text-center">
                            {org.players_targeted}
                          </td>
                          <td className="px-2 py-1 text-center">
                            <span
                              className={
                                org.budget_utilization_pct > 0.9
              ...
              {reportTab === "demand" && (
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <select
                      className="rounded-sm border px-2 py-1 text-xs dark:bg-gray-800 dark:border-gray-600"
                        ...
                        <tbody>
                          {demandPlayers.map((p, idx) => (
                            ...
                            >
                              ...
                              <td className="px-2 py-1 text-center text-yellow-500">
                                {STAR_DISPLAY(p.star_rating)}
                              </td>
                              <td className="px-2 py-1 text-center text-xs">
                                {p.ptype}
                              </td>
                              <td className="px-2 py-1 text-center">
                                {p.num_orgs_targeting}
                              </td>
                              <td className="px-2 py-1 text-right">
                                {p.total_interest}
                              </td>
              ...
              {reportTab === "org-detail" && orgDetail && (
                <div>
                  ...
                  {orgDetail.commitments?.length > 0 && (
                    <div className="mb-2">
                      <p className="text-xs font-semibold text-gray-500 mb-1">
                        Commitments ({orgDetail.commitments.length})
                      </p>
                      <ul className="text-sm space-y-0.5">
                        {orgDetail.commitments.map((c) => (
                          <li key={c.player_id}>
                            <span className="text-yellow-500">
                              {STAR_DISPLAY(c.star_rating)}
                            </span>{" "}
                            {c.player_name} (Wk {c.week_committed},{" "}
                            {c.points_total} pts)
                          </li>
                        ))}
                      </ul>
                  ...
                  {orgDetail.investments?.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-gray-500 mb-1">
                        Active Investments ({orgDetail.investments.length})
                      </p>
                      <ul className="text-sm space-y-0.5">
                        {orgDetail.investments.map((inv) => (
                          ...
                          >
                            <span className="text-yellow-500">
                              {STAR_DISPLAY(inv.star_rating)}
                            </span>{" "}
                            {inv.player_name} &mdash; {inv.invested_points} pts
                            ({inv.status})
                          </li>
                        ))}
                      </ul>
