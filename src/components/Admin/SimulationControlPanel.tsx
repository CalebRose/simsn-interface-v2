...
export const SimulationControlPanel: FC<SimulationControlPanelProps> = ({
  leagueYearId,
}) => {
  ...
  const loadTimestamp = useCallback(async () => {
    try {
      ...
      enqueueSnackbar(e?.message || "Failed to load timestamp", {
        variant: "error",
      });
    }
    setIsLoading(false);
  ...
  const confirmAndExec = useCallback(
  ...
  const allGamesRan =
    ts.GamesARan && ts.GamesBRan && ts.GamesCRan && ts.GamesDRan;
      ...
      <Border classes="w-full mt-4">
        <div className="p-4">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <span
              className={`px-3 py-1 rounded-full text-xs font-semibold ${PHASE_COLORS[phase]}`}
            >
              {PHASE_LABELS[phase]}
            </span>
          ...
          {phase === "REGULAR_SEASON" && (
            <div className="flex items-center gap-2 mb-4">
              ...
              {SUBWEEK_LABELS.map((label, i) => (
                <span
                  key={label}
                  className={`inline-flex items-center justify-center w-8 h-8 rounded-sm text-xs font-semibold border ${
            ...
            <div className="border rounded-sm p-3 mb-3 dark:border-gray-700">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  ...
                  disabled={isBusy || ts.RunGames}
                  onClick={() =>
                    confirmAndExec("Reset Week", async () => {
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {SUBWEEK_LABELS.map((label, i) => (
                  <Button
                    ...
                    onClick={() =>
                      exec(`Simulate Subweek ${label}`, async () => {
          ...
          {/* Phase Control */}
          <div className="border rounded-sm p-3 dark:border-gray-700">
            <Text variant="body-small" classes="font-semibold mb-2">
              Phase Control
            </Text>
            <div className="flex flex-wrap gap-2">
              ...
              {phase === "OFFSEASON" && (
                <>
                  ...
                  <Button
                    ...
                    disabled={isBusy}
                    onClick={() =>
                      confirmAndExec("Start New Season", async () => {
                        await BaseballService.StartNewSeason({
                          league_year_id: leagueYearId + 1,
                        });
                      })
                    }
                  >
              ...
              {phase === "FREE_AGENCY" && (
                <>
                  ...
                  <Button
                    ...
                    disabled={isBusy}
                    onClick={() =>
                      confirmAndExec("End Free Agency", async () => {
              ...
              {phase === "DRAFT" && (
                <Button
                  ...
                  disabled={isBusy}
                  onClick={() =>
                    confirmAndExec("End Draft", async () => {
              ...
              {phase === "RECRUITING" && (
                <Button
                  ...
                  disabled={isBusy}
                  onClick={() =>
                    confirmAndExec("Lock Recruiting", async () => {
