...
export const HSScoutingContent: FC<HSScoutingContentProps> = ({
  ...
  const potFuzzed = player.visibility_context
    ? !player.visibility_context.potentials_precise
    : true;
  const isPitcher = bio.ptype === "Pitcher";
  const cls = getHSClassYear(bio.age);

  // ── Recruiting detail (fetched in parallel) ──
  const [recruitDetail, setRecruitDetail] =
    useState<RecruitingPlayerDetail | null>(null);
  useEffect(() => {
        ...
        <div className="flex items-start justify-between mb-2">
          <div>
            <div className="flex items-center gap-2">
              <span
                className={`px-2 py-0.5 text-xs rounded-sm font-semibold ${isPitcher ? "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300" : "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300"}`}
              >
          ...
          {recruitDetail && (
            <div className="flex items-center gap-1">
              <span className="text-yellow-500 text-sm">
                {"★".repeat(recruitDetail.star_rating)}
              </span>
              <span className="text-gray-600 text-sm">
                {"★".repeat(5 - recruitDetail.star_rating)}
              </span>
              <span className="text-xs text-gray-400 ml-1">
                #{recruitDetail.rank_overall}
              </span>
            </div>
          )}
        </div>
          ...
          <div className="mt-2 flex flex-wrap gap-1">
            {[
              ...
              .map((name, i) => (
                <span
                  key={i}
                  className="text-xs px-2 py-0.5 rounded-sm bg-gray-100 dark:bg-gray-700 font-medium"
                >
                  {name}
                </span>
              ))}
      ...
      <Border classes="p-3">
        ...
        <div className="space-y-2">
          {HS_ACTION_PROGRESSION.map(({ action, prereq }) => {
            ...
            const canAfford = scoutingBudget
              ? scoutingBudget.remaining_points >= cost
              : true;
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  {!isUnlocked && (
                    <span className="text-xs text-gray-400">({cost} pts)</span>
                  )}
                </div>
                {isAvailable && (
                  <PillButton
                ...
                {isLocked && prereq && (
                  <span className="text-xs text-gray-400">
                    Requires: {SCOUTING_ACTION_LABELS[prereq] ?? prereq}
                  </span>
                )}
              </div>
            );
        ...
        <Border classes="p-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            {player.text_report.fielding && (
              <ReportCard title="Fielding" text={player.text_report.fielding} />
            )}
            {player.text_report.pitching && (
              <ReportCard title="Pitching" text={player.text_report.pitching} />
            )}
            {player.text_report.athletic && (
              <ReportCard title="Athletic" text={player.text_report.athletic} />
            )}
          </div>
        </Border>
      )}
      ...
      {potentials && Object.values(potentials).some((v) => v != null) && (
        <Border classes="p-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {(isPitcher ? PITCH_POTENTIAL_GROUPS : POS_POTENTIAL_GROUPS).map(
              (group) => {
                const visibleKeys = group.keys.filter(
                  (k) => potentials[k.key] != null,
                );
                      ...
                      {visibleKeys.map(({ key, label }) => {
                        ...
                        return (
                          ...
                          >
                            <span className="text-gray-400">{label}</span>
                            <span
                              className={`font-semibold ${gradeColor(val)}`}
                            >
                              {val}
                            </span>
                          </div>
      ...
      {player.generated_stats && (
        <Border classes="p-3">
          ...
          {player.generated_stats.fielding && (
            <GeneratedFieldingSection stats={player.generated_stats.fielding} />
          )}
          {player.generated_stats.pitching && (
            <GeneratedPitchingSection stats={player.generated_stats.pitching} />
          )}
        </Border>
      )}
      ...
      {recruitDetail && (
        <Border classes="p-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1 text-sm">
            <BioField
              label="Star Rating"
              value={`${"★".repeat(recruitDetail.star_rating)} (${recruitDetail.star_rating})`}
            />
            <BioField
              label="Overall Rank"
              value={`#${recruitDetail.rank_overall}`}
            />
            <BioField
              label="Position Rank"
              value={`#${recruitDetail.rank_by_ptype}`}
            />
            <BioField
              label="This Week"
              value={`${recruitDetail.your_investment_this_week} pts`}
            />
            <BioField
              label="Total Invested"
              value={`${recruitDetail.your_investment} pts`}
            />
            <div>
              <span className="text-gray-400 text-xs">Interest: </span>
              <span
                className={`text-xs font-semibold ${
                  recruitDetail.interest_gauge === "Very High"
                    ? "text-green-500"
                    : recruitDetail.interest_gauge === "High"
                      ? "text-blue-500"
                      : recruitDetail.interest_gauge === "Medium"
                        ? "text-yellow-500"
                        : "text-gray-400"
                }`}
              >
          ...
          {recruitDetail.commitment && (
            <div className="mt-2 px-3 py-1.5 rounded-sm bg-gray-50 dark:bg-gray-700/50 text-sm">
              Committed to{" "}
              <span className="font-semibold">
                {recruitDetail.commitment.org_abbrev}
              </span>{" "}
              in week {recruitDetail.commitment.week_committed}
            </div>
          )}
        </Border>
      )}
