...
import { CollegePoolResponse, PoolPlayer, ScoutingBudget } from "../../../models/baseball/baseballScoutingModels";
...
const StarDisplay = ({ rating }: { rating: number | undefined }) => {
  ...
  return (
    <>
      ...
      <span className="text-gray-600 dark:text-gray-500">{"★".repeat(5 - rating)}</span>
    </>
  );
};
...
interface BaseballRecruitingPageProps { league: string }
...
export const BaseballRecruitingPage = (_props: BaseballRecruitingPageProps) => {
  ...
  const fetcher = useCallback(
    (params: Record<string, any>) =>
      BaseballService.GetCollegePool({ ...params, viewing_org_id: orgId, league_year_id: leagueYearId, age: 18 }) as Promise<CollegePoolResponse>,
    [orgId, leagueYearId],
  );
  ...
  // ── Recruiting state ──
  const [recruitingState, setRecruitingState] = useState<RecruitingState | null>(null);
  useEffect(() => {
  ...
  const loadBoard = useCallback(() => {
    ...
    BaseballService.GetRecruitingBoard(orgId, leagueYearId)
      ...
      .catch((err) => { console.error("[RecruitingBoard] fetch failed:", err, "status:", err?.status); setBoardPlayers([]); setBoardPlayerIds(new Set()); })
  ...
  const handleAddToBoard = useCallback(async (playerId: number) => {
    try {
      await BaseballService.AddToBoard({ org_id: orgId, league_year_id: leagueYearId, player_id: playerId });
  ...
  const handleRemoveFromBoard = useCallback(async (playerId: number) => {
    try {
      await BaseballService.RemoveFromBoard({ org_id: orgId, league_year_id: leagueYearId, player_id: playerId });
      ...
      .then((data) => {
        if (data.potentials && Object.values(data.potentials).some((v) => v != null && v !== "?")) {
          const preciseUnlocked =
            ...
            data.visibility?.unlocked?.includes("recruit_potential_precise") ||
  ...
  }, [scoutingModal, selectedPlayerId, cachePlayerScouting, setBudgetRefreshKey]);

  // ── Investment data ──
  const [investState, setInvestState] = useState<InvestmentStateResponse | null>(null);
  const [investLoading, setInvestLoading] = useState(false);
  const [investAllocations, setInvestAllocations] = useState<Record<number, number>>({});
  ...
  const loadInvestments = useCallback(() => {
    ...
    BaseballService.GetInvestments(orgId, leagueYearId)
      .then((r) => {
        ...
        r.investments.forEach((inv) => { allocs[inv.player_id] = inv.points; });
  ...
  const investSpent = useMemo(() => {
    return Object.values(investAllocations).reduce((sum, v) => sum + (v || 0), 0);
  }, [investAllocations]);
  ...
  const handleInvestChange = useCallback((playerId: number, value: number) => {
    const clamped = Math.max(0, Math.min(investMaxPerPlayer, value));
    setInvestAllocations((prev) => {
      const next = { ...prev };
      if (clamped === 0) { delete next[playerId]; } else { next[playerId] = clamped; }
      return next;
    });
  }, [investMaxPerPlayer]);
  ...
  const handleSaveInvestments = useCallback(async () => {
    ...
    if (investRemaining < 0) {
      enqueueSnackbar(`Over budget by ${Math.abs(investRemaining)} points`, { variant: "error" });
      return;
    }
    ...
    try {
      ...
      if (resp.errors?.length) {
        resp.errors.forEach((e) => enqueueSnackbar(`Player ${e.player_id}: ${e.error}`, { variant: "error" }));
      }
      if (resp.accepted?.length) {
        enqueueSnackbar(`Investments saved (${resp.budget_remaining} pts remaining)`, { variant: "success", autoHideDuration: 2000 });
      }
      loadInvestments();
    } catch (err: any) {
      enqueueSnackbar(err?.message || "Failed to save investments", { variant: "error" });
    } finally {
      setInvestSaving(false);
    }
  }, [investState, investAllocations, investRemaining, orgId, leagueYearId, enqueueSnackbar, loadInvestments]);
  ...
  const loadCommitments = useCallback(() => {
    ...
    BaseballService.GetCommitments({ league_year_id: leagueYearId, page: commitmentsPage, per_page: 50 })
      .then((r) => { setCommitments(r.commitments); setCommitmentsTotalPages(r.total_pages); })
  ...
  const renderPosRow = (pp: PoolPlayer) => {
    ...
    >
      ...
      <td className={`${cell} text-center`}><StarDisplay rating={pp.star_rating} /></td>
  ...
  return (
    <PageContainer>
      <div className="flex-col w-[95vw] sm:w-[90vw] md:w-full md:mb-6 px-2">
        ...
        {recruitingState && (
          <div className={`px-4 py-2 mb-2 rounded text-sm ${
            ...
            {recruitingState.status === "active" && `Recruiting Week ${recruitingState.current_week} of ${recruitingState.total_weeks}`}
            {recruitingState.status === "pending" && "Recruiting has not started yet"}
        ...
        <Border classes="p-4 mb-2" styles={{ borderTop: `3px solid ${headerColor}` }}>
          <div className="mb-4">
            <ButtonGroup>
              ...
              <PillButton variant="primaryOutline" isSelected={activeTab === "commitments"} onClick={() => setActiveTab("commitments")}>
          ...
          {activeTab === "pool" && (
            <>
              ...
              {/* Star Rating */}
              <div className="mb-3">
                <Text variant="small" classes="font-semibold mb-1">Star Rating</Text>
                <ButtonGroup>
                  <PillButton variant="primaryOutline" isSelected={!pool.filters.star_rating} onClick={() => pool.setFilter("star_rating", undefined)}>
                    <Text variant="small">All</Text>
                  </PillButton>
                  {[5, 4, 3, 2, 1].map((s) => (
                    <PillButton key={s} variant="primaryOutline" isSelected={pool.filters.star_rating === s} onClick={() => pool.setFilter("star_rating", s)}>
        ...
        <Border classes="p-4" styles={{ borderTop: `3px solid ${headerColor}` }}>
            ...
            <>
              {pool.isLoading ? (
                <div className="flex items-center justify-center py-12">
                  <Text variant="body" classes="text-gray-500 dark:text-gray-400">Loading recruits...</Text>
                </div>
              ) : pool.error ? (
                <div className="flex items-center justify-center py-12">
                  <Text variant="body" classes="text-gray-500 dark:text-gray-400">Recruiting data is not yet available.</Text>
                </div>
              ) : pool.data.length === 0 ? (
                ...
                <>
                  ...
                  {investState?.status === "active" && (
                    <div className="mb-4 p-3 rounded bg-gray-50 dark:bg-gray-700/50">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Text variant="small" classes={investRemaining < 0 ? "text-red-500 font-semibold" : "text-gray-500 dark:text-gray-400"}>
                          {investSpent} / {investBudget} used ({investRemaining} remaining)
                        </Text>
                      </div>
                      <div className="w-full h-2 bg-gray-200 dark:bg-gray-600 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${Math.min(100, (investSpent / investBudget) * 100)}%` }}
                        />
                      </div>
                  ...
                  {investState?.status === "pending" && (
                    <div className="mb-4 p-3 rounded bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-300 text-sm">
                      Recruiting has not started yet. Point investment will be available once recruiting is active.
                    </div>
                  )}
                  ...
                  <div className="baseball-table-wrapper overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {boardPlayers.map((bp) => {
                          const allocation = investAllocations[bp.player_id] ?? 0;
                          const isCommitted = bp.status === "committed";
                          return (
                          <tr key={bp.player_id} className="border-b border-gray-800 hover:bg-gray-700/30 cursor-pointer"
                          ...
                          >
                            ...
                            <td className={td}>
                              {bp.star_rating != null ? (<>
                                <span className="text-yellow-500">{"★".repeat(bp.star_rating)}</span>
                                <span className="text-gray-600">{"★".repeat(5 - bp.star_rating)}</span>
                              </>) : <span className="text-gray-500">—</span>}
                            </td>
                            <td className={td}>{bp.rank_overall != null ? `#${bp.rank_overall}` : "—"}</td>
                            <td className={td}>{bp.ptype === "Pitcher" ? "P" : "Pos"}</td>
                            <td className={td} onClick={(e) => e.stopPropagation()}>
                              {investState?.status === "active" && !isCommitted ? (
                                <input
                                  type="number"
                                  min={0}
                                  max={investMaxPerPlayer}
                                  value={allocation}
                                  onChange={(e) => handleInvestChange(bp.player_id, parseInt(e.target.value) || 0)}
                            ...
                            <td className={`${td} font-semibold`}>{bp.your_points}</td>
                            <td className={td}>
                              <span className={`px-2 py-0.5 text-xs rounded-full ${
                                bp.interest_gauge === "Very High" ? "bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300" :
                                bp.interest_gauge === "High" ? "bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300" :
                                bp.interest_gauge === "Medium" ? "bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-300" :
                            ...
                            <td className={td}>
                              {isCommitted && bp.committed_to
                                ? <span className="text-xs text-gray-400">Committed to {bp.committed_to.org_abbrev}</span>
                  ...
                  {investState?.status === "active" && (
                    <div className="flex items-center justify-end gap-3 mt-4">
                      {investRemaining < 0 && (
                        <Text variant="small" classes="text-red-500">Over budget by {Math.abs(investRemaining)} pts</Text>
                      )}
                      {investRemaining > 0 && (
                        <Text variant="small" classes="text-gray-400">{investRemaining} pts unallocated</Text>
                      )}
                      <button
          ...
          {activeTab === "commitments" && (
            <>
              {commitmentsLoading ? (
                <div className="flex items-center justify-center py-12">
                  <Text variant="body" classes="text-gray-500 dark:text-gray-400">Loading commitments...</Text>
                </div>
                ...
                <>
                  <div className="baseball-table-wrapper overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {commitments.map((c) => (
                          <tr key={c.player_id} className="border-b border-gray-800 hover:bg-gray-700/30">
                            <td className={`${td} font-medium`}>{c.player_name}</td>
                            <td className={td}>{c.org_abbrev}</td>
                            <td className={td}>
                              <span className="text-yellow-500">{"★".repeat(c.star_rating)}</span>
                              <span className="text-gray-600">{"★".repeat(5 - c.star_rating)}</span>
                            </td>
                            <td className={td}>{c.ptype === "Pitcher" ? "P" : "Pos"}</td>
                  ...
                  <PoolPagination page={commitmentsPage} totalPages={commitmentsTotalPages} onPageChange={setCommitmentsPage} />
                </>
              )}
            </>
          )}
