    ...
    <div className="flex flex-wrap gap-0.5 items-center">
      ...
      {!isCollege && (
        <button
          ...
          onClick={() => onScouting(player, "pro_attrs_precise")}
          title="Scout Precise Attributes"
        >
          Attrs
        </button>
      )}
      <button
        ...
        onClick={() =>
          onScouting(
            player,
            isCollege ? "college_potential_precise" : "pro_potential_precise",
          )
        }
  ...
  const fetchScoutingOverlay = useCallback(
    async (players: Player[], orgId: number, lyId: number) => {
      ...
      await Promise.all(
        players.map(async (p) => {
          try {
            const data = await BaseballService.GetScoutedPlayer(
              p.id,
              orgId,
              lyId,
            );
            ...
            /* player may not be in scouting pool */
          }
        }),
      );
  ...
  const handleScoutingModalClose = useCallback(() => {
    ...
    if (scoutingPlayerId > 0 && effectiveOrgId && leagueYearId) {
      if (isCollege) {
        ...
        BaseballService.GetScoutedPlayer(
          scoutingPlayerId,
          effectiveOrgId,
          leagueYearId,
        )
