...
import { Player, BaseballTeam, PlayerRatings, PlayerPotentials, VisibilityContext, ListedPositionResponse } from "../../../models/baseball/baseballModels";
import {
  SimCollegeBaseball, SimMLB,
  Attributes, Potentials, Contracts,
} from "../../../_constants/constants";
import { getLogo } from "../../../_utility/getLogo";
import { useSimBaseballStore } from "../../../context/SimBaseballContext";
import { useAuthStore } from "../../../context/AuthContext";
import { displayLevel, displayTeamName, LEVEL_ORDER, normalizePlayer } from "../../../_utility/baseballHelpers";
import { useModal } from "../../../_hooks/useModal";
import { BaseballService } from "../../../_services/baseballService";
import { BaseballTransactionModal, ScoutingConfirmationModal, TransactionAction, getTransactionOptions } from "./BaseballTransactionModals";
import {
  type BaseballCategory, type SortConfig, type PlayerStatsMap, comparePlayers,
  AllPlayersTable, PositionTable, PitcherTable,
} from "./BaseballRosterTable";
import {
  BattingLeaderRow, PitchingLeaderRow,
...
const actionBtn = "px-1.5 py-0.5 rounded text-[10px] font-semibold leading-tight whitespace-nowrap disabled:opacity-30 disabled:cursor-not-allowed transition-colors";
...
const QuickActionButtons = ({
  ...
  return (
    <div className="flex flex-wrap gap-0.5 items-center">
      ...
      {isOwnOrg && canMove && (
        <button className={`${actionBtn} bg-green-600/20 text-green-400 hover:bg-green-600/40`} onClick={() => onTransaction(player, "promote")} title="Move to Level">
          Move
        </button>
      )}
      {isOwnOrg && hasContract && !onIR && (
        <button className={`${actionBtn} bg-green-600/20 text-green-400 hover:bg-green-600/40`} onClick={() => onTransaction(player, "ir_place")} title="Place on IR">
          IR
        </button>
      )}
      {isOwnOrg && hasContract && onIR && (
        <button className={`${actionBtn} bg-green-600/20 text-green-400 hover:bg-green-600/40`} onClick={() => onTransaction(player, "ir_activate")} title="Activate from IR">
          Act
        </button>
      )}
      ...
      {isOwnOrg && hasContract && (
        <>
          <button className={`${actionBtn} bg-red-600/20 text-red-400 hover:bg-red-600/40`} onClick={() => onTransaction(player, "release")} title="Release">
            Rel
          </button>
          <button className={`${actionBtn} bg-orange-600/20 text-orange-400 hover:bg-orange-600/40`} onClick={() => onTransaction(player, "extend")} title="Extend">
            Ext
          </button>
          <button className={`${actionBtn} bg-orange-600/20 text-orange-400 hover:bg-orange-600/40`} onClick={() => onTransaction(player, "buyout")} title="Buyout">
            Buy
          </button>
        </>
      )}
...
const TeamSelectorCard = ({ isSelected, onClick, label, sublabel, logoSrc, playerCount, accentColor }: {
  isSelected: boolean; onClick: () => void; label: string;
  sublabel?: string; logoSrc?: string; playerCount?: number; accentColor?: string;
}) => (
  <button
    onClick={onClick}
  ...
  >
    {logoSrc && (
      <img src={logoSrc} className="w-8 h-8 object-contain shrink-0" alt=""
        onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
    )}
    <div className="flex flex-col items-start text-left">
      <span className="text-sm font-medium leading-tight" style={isSelected && accentColor ? { color: accentColor } : undefined}>{label}</span>
      {sublabel && <span className="text-xs text-gray-500 dark:text-gray-400">{sublabel}</span>}
      {playerCount !== undefined && <span className="text-xs text-gray-400 dark:text-gray-500">{playerCount} players</span>}
    </div>
  </button>
);
...
interface BaseballTeamPageProps { league: string }

export const BaseballTeamPage = ({ league }: BaseballTeamPageProps) => {
  const { currentUser } = useAuthStore();
  const { organizations, mlbOrganization, collegeOrganization, loadBootstrapForOrg, seasonContext } = useSimBaseballStore();
  ...
  const leagueOrgs = useMemo(() => {
    return (organizations ?? []).filter((o) => o.league === leagueKey).sort((a, b) => a.org_abbrev.localeCompare(b.org_abbrev));
  ...
  const orgOptions = useMemo(() => {
    const allEntry: SelectOption = { value: ALL_ORGS, label: "All Organizations" };
    ...
    if (isCollege) {
      ...
      const groups = Object.keys(conferenceMap).sort().map((conf) => ({
        label: conf,
    ...
    for (const org of leagueOrgs) {
      ...
      opts.push({ value: String(org.id), label: t?.team_full_name || org.org_abbrev });
    }
    return opts;
  ...
  const selectedOrgOption = useMemo(() => {
    ...
    const flat = Array.isArray(orgOptions) && orgOptions.length > 0 && "options" in orgOptions[0]
      ? (orgOptions as { label: string; options: SelectOption[] }[]).flatMap((g) => g.options)
  ...
  const loadAllOrgPlayers = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(leagueOrgs.map((org) => BaseballService.GetBootstrapLandingData(org.id)));
      setAllOrgPlayers(results.flatMap((d) => {
    ...
    } catch (e) { console.error("Failed to load all org players", e); }
    setIsLoadingAll(false);
  }, [leagueOrgs]);

  // --- Filters & category ---
  const [filterLevel, setFilterLevel] = useState<string>(league === SimMLB ? "mlb" : "all");
  ...
  const [playerStatsMap, setPlayerStatsMap] = useState<PlayerStatsMap>(new Map());
  ...
  const teamColors = useTeamColors(activeColorTeam?.color_one ?? undefined, activeColorTeam?.color_two ?? undefined, activeColorTeam?.color_three ?? undefined);
  ...
  useEffect(() => { setSortConfig(null); }, [category]);
  ...
  const handleTransactionAction = useCallback((player: Player, action: TransactionAction) => {
    setTxnPlayer(player);
    setTxnAction(action);
  ...
  const [scoutConfirmPlayer, setScoutConfirmPlayer] = useState<Player | null>(null);
  const [scoutConfirmAction, setScoutConfirmAction] = useState<ScoutingActionType | null>(null);

  const handleScoutingQuickAction = useCallback((player: Player, actionType: ScoutingActionType) => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const fetches = teamIds.flatMap((tid) => [
          BaseballService.GetBattingLeaders({ league_year_id: leagueYearId, team_id: tid, page_size: 200 })
            .then((res) => { for (const row of res.leaders) map.set(row.player_id, row); })
            .catch(() => {}),
          BaseballService.GetPitchingLeaders({ league_year_id: leagueYearId, team_id: tid, page_size: 200 })
            .then((res) => { for (const row of res.leaders) map.set(row.player_id, row); })
            .catch(() => {}),
        ]);
  ...
  const handlePositionOverride = useCallback(async (playerId: number, positionCode: string | null) => {
    if (!viewedOrg?.teams) return;
    // Find player in pageRosterMap to get their league_level → team_id
    const player = Object.values(pageRosterMap).flat().find((p) => p.id === playerId);
    if (!player) return;
    ...
    try {
      ...
      if (positionCode) {
        res = await BaseballService.SetListedPosition(team.team_id, playerId, positionCode);
      } else {
        res = await BaseballService.ClearListedPosition(team.team_id, playerId);
      }
  ...
  const allPlayers = useMemo(() => {
    ...
    else if (Object.keys(pageRosterMap).length > 0) players = Object.values(pageRosterMap).flat();
    else players = [];
  ...
  const levelTeams = useMemo(() => {
    ...
    const order = league === SimCollegeBaseball
      ? Object.keys(viewedOrg.teams)
      : LEVEL_ORDER;
    return order.filter((l) => viewedOrg.teams[l] || avail.has(l))
  ...
  const filteredPlayers = useMemo(() => {
    ...
    if (sortConfig) return [...filtered].sort((a, b) => comparePlayers(a, b, sortConfig, playerStatsMap));
    const li = (level: string) => { const i = LEVEL_ORDER.indexOf(level); return i === -1 ? LEVEL_ORDER.length : i; };
  ...
  }, [allPlayers, filterLevel, filterType, searchTerm, sortConfig, playerStatsMap]);
  ...
  const pageTitle = useMemo(() => {
    ...
    return t?.team_full_name ? `${t.team_full_name} Organization` : `${viewedOrg.org_abbrev} Organization`;
  }, [viewedOrg, isAllView, league]);

  const logo = useMemo(() => {
    if (isAllView || !viewedOrg?.teams) return "";
    if (league === SimMLB) { const t = viewedOrg.teams["mlb"]; if (t) return getLogo(SimMLB, t.team_id, currentUser?.isRetro); }
    if (league === SimCollegeBaseball) { const es = Object.values(viewedOrg.teams); if (es.length > 0) return getLogo(SimCollegeBaseball, es[0].team_id, currentUser?.isRetro); }
    return "";
    ...
    <PageContainer>
      <div className="flex-col w-[95vw] sm:w-[90vw] md:w-full md:mb-6 px-2">
        ...
        >
          ...
          <div className="ml-auto">
            <SelectDropdown
              ...
              onChange={(opt) => { if (opt) handleOrgChange((opt as SelectOption).value); }}
        ...
        <Border classes="p-4 mb-2" styles={{ borderTop: `3px solid ${headerColor}` }}>
          ...
          {!isAllView && viewedOrg && (
            <div className="mb-4">
              <Text variant="small" classes="font-semibold mb-2">Team</Text>
              <div className="flex flex-wrap gap-2">
                <TeamSelectorCard isSelected={filterLevel === "all"} onClick={() => setFilterLevel("all")} label="All Teams" sublabel={`${allPlayers.length} players`} accentColor={headerColor} />
                {levelTeams.map(({ level, team }) => (
                  <TeamSelectorCard
                    key={level} isSelected={filterLevel === level} onClick={() => setFilterLevel(level)}
                    label={team ? displayTeamName(level, team, viewedOrg.org_abbrev) : displayLevel(level)}
                    sublabel={displayLevel(level)}
                    logoSrc={team ? getLogo(league === SimMLB ? SimMLB : SimCollegeBaseball, team.team_id, currentUser?.isRetro) : undefined}
                    playerCount={allPlayers.filter((p) => p.league_level === level).length}
          ...
          {isAllView && !isLoadingAll && availableLevels.length > 0 && (
            <div className="mb-3">
              ...
              <ButtonGroup>
                <PillButton variant="primaryOutline" isSelected={filterLevel === "all"} onClick={() => setFilterLevel("all")}><Text variant="small">All</Text></PillButton>
                {availableLevels.map((lv) => (
                  <PillButton key={lv} variant="primaryOutline" isSelected={filterLevel === lv} onClick={() => setFilterLevel(lv)}><Text variant="small">{displayLevel(lv)}</Text></PillButton>
                ))}
              </ButtonGroup>
            </div>
          )}
          ...
          <div className="mb-3">
            ...
            <ButtonGroup>
              <PillButton variant="primaryOutline" isSelected={filterType === "all"} onClick={() => setFilterType("all")}><Text variant="small">All</Text></PillButton>
              <PillButton variant="primaryOutline" isSelected={filterType === "Pitcher"} onClick={() => setFilterType("Pitcher")}><Text variant="small">Pitcher</Text></PillButton>
              <PillButton variant="primaryOutline" isSelected={filterType === "Position"} onClick={() => setFilterType("Position")}><Text variant="small">Position</Text></PillButton>
            </ButtonGroup>
          </div>
          ...
          <div className="mb-3">
            ...
            <ButtonGroup>
              <PillButton variant="primaryOutline" isSelected={category === Attributes} onClick={() => setCategory(Attributes)}><Text variant="small">Attributes</Text></PillButton>
              <PillButton variant="primaryOutline" isSelected={category === Potentials} onClick={() => setCategory(Potentials)}><Text variant="small">Potentials</Text></PillButton>
              <PillButton variant="primaryOutline" isSelected={category === Contracts} onClick={() => setCategory(Contracts)}><Text variant="small">Contracts</Text></PillButton>
              <PillButton variant="primaryOutline" isSelected={category === Stats} onClick={() => setCategory(Stats)}><Text variant="small">Stats</Text></PillButton>
            </ButtonGroup>
          </div>

          {/* Search + Count */}
          <div className="flex items-center gap-3">
            <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search player..." className="text-sm border rounded px-2 py-1 w-48 dark:bg-gray-700 dark:border-gray-600" />
            <Text variant="small" classes="text-gray-500 dark:text-gray-400">{filteredPlayers.length} players</Text>
            {scoutingLoading && <Text variant="small" classes="text-blue-500">Loading scouting data...</Text>}
            {scoutingBudget && (
              <Text variant="small" classes="text-gray-500 dark:text-gray-400 ml-auto">
                Scouting: {scoutingBudget.remaining_points}/{scoutingBudget.total_points} pts
              </Text>
            )}
          </div>
        </Border>
        ...
        <Border classes="p-4" styles={{ borderTop: `3px solid ${headerColor}` }}>
          {isLoadingAll || isLoadingOrg ? (
            <div className="flex items-center justify-center py-12">
              <Text variant="body" classes="text-gray-500 dark:text-gray-400">{isLoadingAll ? "Loading all organizations..." : "Loading roster..."}</Text>
            </div>
            ...
            <Text variant="body-small" classes="text-gray-500 dark:text-gray-400">No roster data available.</Text>
          ) : showPitchers ? (
            <PitcherTable players={filteredPlayers} orgAbbrev={displayOrgAbbrev} onPlayerClick={openPlayerModal} sortConfig={sortConfig} onSort={handleSort} category={category} renderActions={renderActions} isCollege={isCollege} isFuzzed onPositionOverride={isOwnOrg ? handlePositionOverride : undefined} playerStatsMap={playerStatsMap} />
          ) : showPosition ? (
            <PositionTable players={filteredPlayers} orgAbbrev={displayOrgAbbrev} onPlayerClick={openPlayerModal} sortConfig={sortConfig} onSort={handleSort} category={category} renderActions={renderActions} isCollege={isCollege} isFuzzed onPositionOverride={isOwnOrg ? handlePositionOverride : undefined} playerStatsMap={playerStatsMap} />
          ) : (
            <AllPlayersTable players={filteredPlayers} orgAbbrev={displayOrgAbbrev} onPlayerClick={openPlayerModal} sortConfig={sortConfig} onSort={handleSort} category={category} renderActions={renderActions} isCollege={isCollege} isFuzzed onPositionOverride={isOwnOrg ? handlePositionOverride : undefined} playerStatsMap={playerStatsMap} />
          )}
        </Border>
      </div>
      ...
      {txnPlayer && seasonContext && viewedOrg && (
        <BaseballTransactionModal
          ...
          onClose={() => { txnModal.handleCloseModal(); setTxnPlayer(null); setTxnAction(null); }}
          player={txnPlayer}
          action={txnAction}
          orgId={viewedOrg.id}
      ...
      {scoutConfirmPlayer && scoutConfirmAction && (
        <ScoutingConfirmationModal
          ...
          onClose={() => { scoutConfirmModal.handleCloseModal(); setScoutConfirmPlayer(null); setScoutConfirmAction(null); }}
