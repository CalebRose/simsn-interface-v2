...
const StatusBadge: FC<{ status: string }> = ({ status }) => (
  <span
    className={`px-2 py-0.5 rounded-sm text-xs font-semibold text-white ${STATUS_COLORS[status] ?? "bg-gray-500"}`}
  >
    {status.replace(/_/g, " ")}
  </span>
);
...
const PlayerPickerTable: FC<PlayerPickerTableProps> = ({
  ...
  const [filterType, setFilterType] = useState<"all" | "Pitcher" | "Position">(
    "all",
  );
  ...
  return (
    <div className="flex flex-col">
      ...
      <input
        ...
        className="text-sm border rounded-sm px-2 py-1 w-full mb-2 bg-gray-700 border-gray-600 text-white"
      />
      ...
      {/* Table */}
      <div className="baseball-table-wrapper trade-picker-table overflow-x-auto overflow-y-auto max-h-[35vh] border border-gray-600 rounded-sm">
    ...
    <Border classes="px-3 py-2">
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <Text variant="xs" classes="text-gray-400">
            ...
            <span
              className={
                player.displayovr != null
                  ? ratingColor(Number(player.displayovr))
                  : ""
              }
            >
              {player.displayovr ?? "?"}
      ...
      <div className="flex items-center gap-2 mt-1.5 flex-wrap">
        ...
        <div className="flex items-center gap-0.5">
          <button
            onClick={decrement}
            className="px-1.5 py-0.5 text-sm bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-sm"
          >
            -
          </button>
          <input
            ...
            className="w-14 px-1 py-0.5 text-xs text-center bg-black text-white border border-gray-500 rounded-sm"
          />
          <button
            onClick={increment}
            className="px-1.5 py-0.5 text-sm bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-sm"
...
export const BaseballTradePage: FC<BaseballTradePageProps> = ({ league }) => {
  ...
  const [bootstrapCache, setBootstrapCache] = useState<
  ...
  const [userStatsMap, setUserStatsMap] =
    useState<PlayerStatsMap>(emptyStatsMap);
  const [targetStatsMap, setTargetStatsMap] =
    useState<PlayerStatsMap>(emptyStatsMap);
  ...
  const loadProposals = useCallback(async () => {
    if (!userOrg) return;
    try {
      const data = await BaseballService.GetTradeProposals(
        userOrg.id,
        undefined,
        200,
      );
  ...
  const fetchOrgStats = useCallback(
    async (orgId: number, setter: (m: PlayerStatsMap) => void) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col">
                <PlayerPickerTable
                  ...
                  onStatsNeeded={() =>
                    fetchOrgStats(userOrg.id, setUserStatsMap)
                  }
                />
                    ...
                    <div className="flex flex-col gap-2">
                      {selectedUserPlayers.map((p) => (
                        <SelectedPlayerCard
                          ...
                          onRetentionChange={(pct) =>
                            updateRetention(p.id, pct)
                          }
              ...
              <div className="flex flex-col">
                <PlayerPickerTable
                  ...
                  onStatsNeeded={() =>
                    fetchOrgStats(targetOrgId, setTargetStatsMap)
                  }
                />
                {selectedTargetPlayers.length > 0 && (
                  <div className="mt-3">
                    <Text variant="small" classes="font-semibold mb-1">
                      {orgMap[targetOrgId]?.org_abbrev ?? "Target"} Sends (
                      {selectedTargetPlayers.length})
                    </Text>
                    <div className="flex flex-col gap-2">
