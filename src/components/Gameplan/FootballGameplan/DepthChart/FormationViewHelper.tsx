...
export type FormationType =
  ...
  | typeof EMPTY
  | typeof WING_T
  | typeof WING_T_DOUBLE_TIGHT
  | typeof SPLIT_BACK_GUN
  | typeof FLEXBONE_GUN
  | typeof WING_SPLIT_BACK_GUN
  | typeof DOUBLE_WING_SPLIT
  | typeof DOUBLE_WING_TIGHT
  | typeof DOUBLE_WING_SPREAD
  | typeof DOUBLE_WING_STRONG
  | typeof DOUBLE_WING
  | typeof SINGLEBACK;

export type DefensiveFormationType =
  | typeof FOUR_THREE_BASE
  | typeof FOUR_THREE_OVER
  | typeof FOUR_THREE_UNDER
  | typeof FOUR_THREE_LIGHT
  | typeof FOUR_THREE_HEAVY
  | typeof THREE_FOUR_BASE
  | typeof THREE_FOUR_OVER
  | typeof THREE_FOUR_UNDER
  | typeof THREE_FOUR_OKIE
  | typeof THREE_FOUR_BRONCO
  | typeof THREE_FOUR_EAGLE
  | typeof THREE_FOUR_FOUR_HEAVY
  | typeof FOUR_FOUR_BASE
  | typeof FOUR_FOUR_OVER
  | typeof FOUR_FOUR_UNDER
  | typeof FOUR_FOUR_JUMBO
  | typeof FOUR_FOUR_HEAVY
  | typeof FOUR_TWO_FIVE_BASE
  | typeof FOUR_TWO_FIVE_NICKEL
  | typeof THREE_THREE_FIVE_BASE
  | typeof THREE_THREE_FIVE_OVER
  | typeof THREE_THREE_FIVE_NICKEL
  | typeof DIME_FOUR_ONE_SIX
  | typeof DIME_THREE_TWO_SIX
  | typeof FOUR_ONE_SIX_DIME
  | typeof FOUR_ONE_SIX_BIG_DIME
  | typeof THREE_TWO_SIX_DIME
  | typeof THREE_TWO_SIX_BIG_PENNY
  | typeof THREE_TWO_SIX_PENNY
  | typeof QUARTER
  | typeof GOAL_LINE;
...
export const getFormationType = (
  ...
  if (
    name.includes(FORMATION_KEYWORDS.SINGLEBACK) &&
    !name.includes(FORMATION_KEYWORDS.GUN)
  ) {
    return SINGLEBACK;
  }

  if (
    name.includes(FORMATION_KEYWORDS.WINGT) &&
    name.includes(FORMATION_KEYWORDS.DOUBLE) &&
    name.includes(FORMATION_KEYWORDS.TIGHT)
  ) {
  ...
  if (
    name.includes(FORMATION_KEYWORDS.DOUBLE) &&
    name.includes(FORMATION_KEYWORDS.WING) &&
    name.includes(FORMATION_KEYWORDS.SPLIT)
  ) {
  ...
  if (
    name.includes(FORMATION_KEYWORDS.DOUBLE) &&
    name.includes(FORMATION_KEYWORDS.WING) &&
    name.includes(FORMATION_KEYWORDS.SPREAD)
  ) {
  ...
  if (
    name.includes(FORMATION_KEYWORDS.DOUBLE) &&
    name.includes(FORMATION_KEYWORDS.WING) &&
    name.includes(FORMATION_KEYWORDS.STRONG)
  ) {
  ...
  if (
    name.includes(FORMATION_KEYWORDS.FLEXBONE) &&
    name.includes(FORMATION_KEYWORDS.GUN)
  ) {
    return FLEXBONE_GUN;
  }
  ...
  if (
    name.includes(FORMATION_KEYWORDS.WING) &&
    name.includes(FORMATION_KEYWORDS.SPLIT) &&
    name.includes(FORMATION_KEYWORDS.BACK) &&
    name.includes(FORMATION_KEYWORDS.GUN)
  ) {
  ...
  if (
    name.includes(FORMATION_KEYWORDS.EMPTY) ||
    positions.filter((p) => p.startsWith(POSITION_PREFIXES.WR)).length >= 5
  ) {
    return EMPTY;
  }
  ...
  if (
    name.includes(FORMATION_KEYWORDS.GUN) ||
    name.includes(`${FORMATION_KEYWORDS.SPREAD} ${FORMATION_KEYWORDS.GUN}`)
  ) {
    return GUN;
  }
  ...
  if (
    name.includes(FORMATION_KEYWORDS.SPREAD) ||
    positions.filter((p) => p.startsWith(POSITION_PREFIXES.WR)).length >= 3
  ) {
    return SPREAD;
  }

  const teCount = positions.filter((p) =>
    p.startsWith(POSITION_PREFIXES.TE),
  ).length;
  const hasFB = positions.some((p) => p.startsWith(POSITION_PREFIXES.FB));
  if (
    name.includes(`${FORMATION_KEYWORDS.DOUBLE} ${FORMATION_KEYWORDS.TIGHT}`) ||
    name.includes(FORMATION_KEYWORDS.HEAVY) ||
    teCount >= 2 ||
    (teCount >= 1 && hasFB)
  ) {
    return TIGHT;
...
export const getDefensiveFormationType = (
  formationName: string,
  positions: string[] = [],
  defensiveScheme?: string,
): DefensiveFormationType => {
  const name = formationName.toLowerCase();

  if (name === "4-1-6 dime" && defensiveScheme) {
    if (
      defensiveScheme === "Speed" &&
      positions.includes(`${POSITION_PREFIXES.SS}2`)
    ) {
      return FOUR_ONE_SIX_DIME;
    } else if (
      defensiveScheme === "4-Man Front Spread Stopper" &&
      !positions.includes(`${POSITION_PREFIXES.SS}2`)
    ) {
      return FOUR_ONE_SIX_DIME;
    }
  ...
  if (
    name.includes(FORMATION_KEYWORDS.DIME) ||
    name.includes("4-1-6") ||
    name.includes("3-2-6")
  ) {
    ...
    if (
      name.includes(`${FORMATION_KEYWORDS.BIG} ${FORMATION_KEYWORDS.PENNY}`)
    ) {
  ...
  if (
    name.includes(FORMATION_KEYWORDS.NICKEL) ||
    name.includes("4-2-5") ||
    name.includes("3-3-5")
  ) {
    if (name === "4-2-5 base") {
  ...
  if (
    name.includes(FORMATION_KEYWORDS.QUARTER) ||
    positions.filter((p) => p.startsWith(POSITION_PREFIXES.CB)).length >= 4
  ) {
    return QUARTER;
  }

  if (
    name.includes(FORMATION_KEYWORDS.GOAL) &&
    name.includes(FORMATION_KEYWORDS.LINE)
  ) {
    return GOAL_LINE;
  }
...
export const getFormationLayout = (formation: Formation): FormationLayout => {
  ...
  const positions: FormationPositionData[] = [
    {
      position: `${POSITION_PREFIXES.QB}1`,
      row: qbRow,
      col: GRID_POSITIONS.COLUMNS[7],
      shouldRender: true,
    },
    {
      position: `${POSITION_PREFIXES.LT}1`,
      row: GRID_POSITIONS.ROWS[3],
      col: GRID_POSITIONS.COLUMNS[5],
      shouldRender: true,
    },
    {
      position: `${POSITION_PREFIXES.LG}1`,
      row: GRID_POSITIONS.ROWS[3],
      col: GRID_POSITIONS.COLUMNS[6],
      shouldRender: true,
    },
    {
      position: `${POSITION_PREFIXES.C}1`,
      row: GRID_POSITIONS.ROWS[3],
      col: GRID_POSITIONS.COLUMNS[7],
      shouldRender: true,
    },
    {
      position: `${POSITION_PREFIXES.RG}1`,
      row: GRID_POSITIONS.ROWS[3],
      col: GRID_POSITIONS.COLUMNS[8],
      shouldRender: true,
    },
    {
      position: `${POSITION_PREFIXES.RT}1`,
      row: GRID_POSITIONS.ROWS[3],
      col: GRID_POSITIONS.COLUMNS[9],
      shouldRender: true,
    },
  ];

  const wrPositions = formation.positions.filter((p) =>
    p.startsWith(POSITION_PREFIXES.WR),
  );
  const tePositions = formation.positions.filter((p) =>
    p.startsWith(POSITION_PREFIXES.TE),
  );
  const rbPositions = formation.positions.filter((p) =>
    p.startsWith(POSITION_PREFIXES.RB),
  );
  const fbPositions = formation.positions.filter((p) =>
    p.startsWith(POSITION_PREFIXES.FB),
  );

  const wrPositioning = getWRPositioning(
    formationType,
    formation.name,
    wrPositions,
  );
  ...
  wrPositions.forEach((wr) => {
    if (wrPositioning[wr]) {
      positions.push({
        ...
        shouldRender: true,
        showBackup:
          wr === `${POSITION_PREFIXES.WR}4` && formationType === EMPTY
            ? true
            : false,
      });
    }
  ...
  const tePositioning = getTEPositioning(
    formationType,
    formation.name,
    tePositions,
  );
...
export const getDefensiveFormationLayout = (
  formation: Formation,
  defensiveScheme?: string,
): FormationLayout => {
  const formationType = getDefensiveFormationType(
    formation.name,
    formation.positions,
    defensiveScheme,
  );
  ...
  const positions: FormationPositionData[] = [];
  const dlPositions = formation.positions.filter((p) =>
    [
      POSITION_PREFIXES.RE,
      POSITION_PREFIXES.DT,
      POSITION_PREFIXES.LE,
      POSITION_PREFIXES.DE,
    ].some((pos) => p.startsWith(pos)),
  );

  const lbPositions = formation.positions.filter((p) =>
    [
      POSITION_PREFIXES.LOLB,
      POSITION_PREFIXES.MLB,
      POSITION_PREFIXES.ROLB,
      POSITION_PREFIXES.OLB,
      POSITION_PREFIXES.ILB,
    ].some((pos) => p.startsWith(pos)),
  );
  const dbPositions = formation.positions.filter((p) =>
    [POSITION_PREFIXES.CB, POSITION_PREFIXES.FS, POSITION_PREFIXES.SS].some(
      (pos) => p.startsWith(pos),
    ),
  );
...
export const getDefensiveLinePositions = (
  formation: Formation,
  defensiveScheme?: string,
): string[] => {
  const formationType = getDefensiveFormationType(
  ...
  let dlPositions = formation.positions.filter((p) =>
    [
      POSITION_PREFIXES.RE,
      POSITION_PREFIXES.DT,
      POSITION_PREFIXES.LE,
  ...
  if (formationType === FOUR_FOUR_OVER || formationType === FOUR_THREE_OVER) {
    const lolbPosition = formation.positions.find(
      (p) => p === `${POSITION_PREFIXES.LOLB}1`,
    );
    if (lolbPosition) {
      dlPositions = [lolbPosition, ...dlPositions];
    }
  } else if (formationType === FOUR_FOUR_UNDER) {
    const rolbPosition = formation.positions.find(
      (p) => p === `${POSITION_PREFIXES.ROLB}1`,
    );
    if (rolbPosition) {
      dlPositions = [...dlPositions, rolbPosition];
    }
  } else if (
    formationType === THREE_FOUR_BRONCO ||
    formationType === THREE_TWO_SIX_DIME
  ) {
    const rolbPosition = formation.positions.find(
      (p) => p === `${POSITION_PREFIXES.ROLB}1`,
    );
    ...
    if (rolbPosition) {
      if (
        defensiveScheme === "2-Gap" &&
        !formation.positions.includes(`${POSITION_PREFIXES.DT}1`)
      ) {
        dlPositions = [...dlPositions, rolbPosition];
      } else if (
    ...
    formationType === THREE_THREE_FIVE_OVER ||
    formationType === THREE_FOUR_EAGLE
  ) {
    const lolbPosition = formation.positions.find(
      (p) => p === `${POSITION_PREFIXES.LOLB}1`,
    );
  ...
  if (formationType.includes("3-") && !formationType.includes("4-3")) {
    return dlPositions.sort((a, b) => {
      const order = [
        `${POSITION_PREFIXES.ROLB}1`,
        `${POSITION_PREFIXES.RE}1`,
        `${POSITION_PREFIXES.DT}1`,
        `${POSITION_PREFIXES.LE}1`,
        `${POSITION_PREFIXES.LOLB}1`,
      ];
      ...
      if (a.startsWith(POSITION_PREFIXES.DT))
        return b.startsWith(POSITION_PREFIXES.LE) ? -1 : 0;
      if (b.startsWith(POSITION_PREFIXES.DT))
        return a.startsWith(POSITION_PREFIXES.LE) ? 1 : 0;
      return 0;
    });
  } else {
    return dlPositions.sort((a, b) => {
      ...
      if (
        a.startsWith(POSITION_PREFIXES.DT) &&
        b.startsWith(POSITION_PREFIXES.DT)
      ) {
...
const addLinebackerPositions = (
  positions: FormationPositionData[],
  lbPositions: string[],
  formationType: DefensiveFormationType,
  ...
  if (
    formationType === FOUR_FOUR_OVER ||
    formationType === FOUR_THREE_OVER ||
    formationType === THREE_THREE_FIVE_OVER ||
    formationType === THREE_FOUR_EAGLE
  ) {
    filteredLbPositions = lbPositions.filter(
      (p) => p !== `${POSITION_PREFIXES.LOLB}1`,
    );
  }
  if (
    formationType === FOUR_FOUR_UNDER ||
    formationType === THREE_FOUR_BRONCO
  ) {
    filteredLbPositions = lbPositions.filter(
      (p) => p !== `${POSITION_PREFIXES.ROLB}1`,
  ...
  if (formationType === THREE_THREE_FIVE_NICKEL && formation) {
    if (
      ...
      filteredLbPositions = lbPositions.filter(
        (p) =>
          p !== `${POSITION_PREFIXES.ROLB}1` &&
          p !== `${POSITION_PREFIXES.LOLB}1`,
      );
    }
  }
...
const addSecondaryPositions = (
  positions: FormationPositionData[],
  dbPositions: string[],
  formationType: DefensiveFormationType,
) => {
  const cbPositions = dbPositions.filter((p) =>
    p.startsWith(POSITION_PREFIXES.CB),
  );
  const safetyPositions = dbPositions.filter(
    (p) =>
      p.startsWith(POSITION_PREFIXES.FS) || p.startsWith(POSITION_PREFIXES.SS),
  );
  ...
  cbPositions.forEach((position, index) => {
    ...
    const cbNumber = parseInt(
      position.match(/\d+$/)?.[0] || (index + 1).toString(),
    );
  ...
  safetyPositions.forEach((position) => {
    ...
    if (
      formationType === FOUR_THREE_HEAVY &&
      position === `${POSITION_PREFIXES.SS}2`
    ) {
      row = 7;
      col = 3;
    } else if (
      formationType === FOUR_FOUR_JUMBO &&
      (position === `${POSITION_PREFIXES.SS}1` ||
        position === `${POSITION_PREFIXES.SS}2`)
    ) {
      row = 7;
      ...
      formationType === FOUR_FOUR_JUMBO &&
      position === `${POSITION_PREFIXES.FS}1`
    ) {
      col = 7;
    } else if (
      formationType === THREE_FOUR_FOUR_HEAVY &&
      position === `${POSITION_PREFIXES.SS}1`
    ) {
      row = 7;
      col = 3;
    } else if (
      formationType === FOUR_FOUR_HEAVY &&
      position === `${POSITION_PREFIXES.SS}1`
    ) {
      row = 7;
      col = 3;
      ...
      if (
        position.startsWith(POSITION_PREFIXES.FS) &&
        formationType !== FOUR_FOUR_JUMBO
      ) {
        col = 5;
...
export const shouldRenderPosition = (
  position: string,
  ...
  if (
    [
      POSITION_PREFIXES.QB,
      POSITION_PREFIXES.LT,
      POSITION_PREFIXES.LG,
      POSITION_PREFIXES.C,
      POSITION_PREFIXES.RG,
      POSITION_PREFIXES.RT,
    ].includes(basePosition as any)
  ) {
    return true;
  }
