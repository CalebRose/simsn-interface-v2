...
import {
  getGameplanPlayerInfo,
  getDepthChartPlayerInfo,
} from "../Utils/GameplanPlayerUtils";
import { Team, Player } from "../../../../_constants/constants";
...
export const OffensiveDistributionsPanel: React.FC<
  OffensiveDistributionsPanelProps
> = ({
  gameplan,
  players,
  depthChart,
  ...
  const [passTypeInfoModal, setPassTypeInfoModal] = useState<string | null>(
    null
  );
  ...
  return (
    <div
      className={`space-y-6 ${className} border-2 p-4 rounded-lg`}
    ...
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          <div className="bg-gray-700 bg-opacity-50 border border-gray-600 rounded-lg p-2">
            <div className="flex items-center justify-between mb-4 ">
              ...
              <Text
                variant="small"
                classes={`font-semibold ${getPercentageStatusColor(
                  passDistTotal
                )}`}
              >
                  ...
                  <div key={fieldName} className="flex items-center gap-2">
                    <div className="flex-1">
                      <GameplanInputSmall
                        ...
                        value={
                          gameplan[fieldName as keyof GameplanData] as number
                        }
                        onChange={(e) =>
                          onChange(fieldName, parseInt(e.target.value) || 0)
                        }
                    ...
                    return (
                      <div className="bg-gray-800 bg-opacity-50 border border-gray-600 rounded-lg p-2">
                        <GameplanSlider
                          ...
                          label="Left vs Right"
                          value={
                            gameplan[label as keyof GameplanData] as number
                          }
                          onChange={(e) =>
                            onChange(label, parseInt(e.target.value) || 0)
                          }
                          min={0}
                          max={100}
                          disabled={disabled || gameplan.DefaultOffense}
                          ...
                          valueLabel={`${
                            gameplan[label as keyof GameplanData]
                          }%`}
                          className="pt-2"
                        />
                      </div>
          ...
          <div className="bg-gray-700 bg-opacity-50 border border-gray-600 rounded-lg p-2">
            ...
            <div className="space-y-3">
              {TargetingLabels.map((label) => {
                ...
                let playerInfo;
                if (depthChart) {
                  playerInfo = getDepthChartPlayerInfo(
                    depthChart,
                    position,
                    level,
                    league
                  ...
                  playerInfo = getGameplanPlayerInfo(
                    players,
                    position,
                    level,
                    league
                ...
                return (
                  <TargetInput
                    ...
                    targetDepth={
                      (gameplan[
                        `TargetDepth${label}` as keyof GameplanData
                      ] as string) || "None"
                    }
                    onTargetDepthChange={(depth) =>
                      onChange(`TargetDepth${label}`, depth)
                    }
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-gray-700 bg-opacity-50 border border-gray-600 rounded-lg p-2 h-3/4">
              ...
              <div className="space-y-3">
                <RunnerInput
                  ...
                  playerInfo={
                    depthChart
                      ? getDepthChartPlayerInfo(depthChart, "QB", 1, league)
                      : getGameplanPlayerInfo(players, "QB", 1, league)
                  }
                  disabled={disabled || gameplan.DefaultOffense}
                ...
                <RunnerInput
                  ...
                  playerInfo={
                    depthChart
                      ? getDepthChartPlayerInfo(depthChart, "RB", 1, league)
                      : getGameplanPlayerInfo(players, "RB", 1, league)
                  }
                  disabled={disabled || gameplan.DefaultOffense}
                ...
                <RunnerInput
                  ...
                  playerInfo={
                    depthChart
                      ? getDepthChartPlayerInfo(depthChart, "RB", 2, league)
                      : getGameplanPlayerInfo(players, "RB", 2, league)
                  }
                  disabled={disabled || gameplan.DefaultOffense}
                ...
                <RunnerInput
                  ...
                  playerInfo={
                    depthChart
                      ? getDepthChartPlayerInfo(depthChart, "RB", 3, league)
                      : getGameplanPlayerInfo(players, "RB", 3, league)
                  }
                  disabled={disabled || gameplan.DefaultOffense}
                ...
                <RunnerInput
                  ...
                  playerInfo={
                    depthChart
                      ? getDepthChartPlayerInfo(depthChart, "FB", 1, league)
                      : getGameplanPlayerInfo(players, "FB", 1, league)
                  }
                  disabled={disabled || gameplan.DefaultOffense}
                ...
                <RunnerInput
                  ...
                  playerInfo={
                    depthChart
                      ? getDepthChartPlayerInfo(depthChart, "FB", 2, league)
                      : getGameplanPlayerInfo(players, "FB", 2, league)
                  }
                  disabled={disabled || gameplan.DefaultOffense}
                ...
                <TargetInput
                  ...
                  playerInfo={(() => {
                    const wrPosition =
                      gameplan.RunnerDistributionWRPosition || "WR1";
                  ...
                  onTargetDepthChange={(position) =>
                    onChange("RunnerDistributionWRPosition", position)
                  }
            ...
            <div className="bg-gray-700 bg-opacity-50 border border-gray-600 rounded-lg p-4">
              ...
              <GameplanSlider
                ...
                onChange={(e) =>
                  onChange("PrimaryHB", parseInt(e.target.value) || 0)
                }
                min={0}
                max={100}
                disabled={disabled || gameplan.DefaultOffense}
      ...
      >
        <div className="space-y-4">
          <Text variant="body" classes="text-gray-300">
            The Offensive Distributions window is where you can configure which
            types of plays are run, and which player is being distributed or
            targeted for the ball in both the run and pass game.
          </Text>
          <div className="space-y-3">
            <div>
              <Text variant="body" classes="text-white font-semibold">
                Target Distribution (Pass Game):
              </Text>
              <Text variant="small" classes="text-gray-300">
                These are weights between 0 and 10 that determine targeting
                probability. The heavier the weight, the more likely the player
                is to be targeted. Formula: Player Weight / Total Weight =
                Target Percentage.
              </Text>
            </div>
            <div>
              <Text variant="body" classes="text-white font-semibold">
                Running Distribution (Run Game):
              </Text>
              <Text variant="small" classes="text-gray-300">
                Weights between 0 and 10 that determine which players carry the
                ball. Similar calculation as targeting.
              </Text>
            </div>
            <div>
              <Text variant="body" classes="text-white font-semibold">
              ...
              <Text variant="small" classes="text-gray-300">
                Controls which type of pass the receiver will most likely run.
                Options: Short, Medium, Long, Deep, or None (no preference).
              </Text>
            </div>
            <div>
              <Text variant="body" classes="text-white font-semibold">
              ...
              <Text variant="small" classes="text-gray-300">
                Run, Option, Pass, and RPO type plays must accumulate to 100 in
                each category. If you have no weights in Option or RPO plays in
                your formations, you can leave those distributions as-is.
              </Text>
            </div>
            <div>
              <Text variant="body" classes="text-white font-semibold">
              ...
              <Text variant="small" classes="text-gray-300">
                Slider between 0 and 100 that controls how much time on the
                field your RB1 will have, in comparison to the second or third
                string.
              </Text>
            </div>
          </div>
      ...
      >
        {passTypeInfoModal && (
          <div className="space-y-4">
            <Text variant="body" classes="text-gray-300">
              Maximum values for {passTypeInfoModal} pass type by offensive
              scheme:
            </Text>
            <div className="space-y-3">
              {passTypeInfoModal &&
