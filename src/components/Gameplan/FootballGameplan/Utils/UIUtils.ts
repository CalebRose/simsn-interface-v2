...
export const getValidationTextColor = (
  hasError: boolean,
  hasWarning: boolean,
): string => {
...
export const getValidationBgColor = (
  hasError: boolean,
  hasWarning: boolean,
): string => {
...
export const getRatingColor = (
  ...
  if (typeof rating === "string") {
    return (
      RATING_COLORS.STRING[rating as keyof typeof RATING_COLORS.STRING] ||
      RATING_COLORS.STRING.F
    );
  }
  if (league === SimNFL) {
    if (rating >= NFL_RATING_THRESHOLDS.ELITE)
      return RATING_COLORS.NUMBER.ELITE;
    if (rating >= NFL_RATING_THRESHOLDS.EXCELLENT)
      return RATING_COLORS.NUMBER.EXCELLENT;
    if (rating >= NFL_RATING_THRESHOLDS.GOOD) return RATING_COLORS.NUMBER.GOOD;
    if (rating >= NFL_RATING_THRESHOLDS.ABOVE_AVERAGE)
      return RATING_COLORS.NUMBER.ABOVE_AVERAGE;
    if (rating >= NFL_RATING_THRESHOLDS.AVERAGE)
      return RATING_COLORS.NUMBER.AVERAGE;
  } else if (league === SimPHL) {
    if (rating >= PHL_RATING_THRESHOLDS.ELITE)
      return RATING_COLORS.NUMBER.ELITE;
    if (rating >= PHL_RATING_THRESHOLDS.EXCELLENT)
      return RATING_COLORS.NUMBER.EXCELLENT;
    if (rating >= PHL_RATING_THRESHOLDS.GOOD) return RATING_COLORS.NUMBER.GOOD;
    if (rating >= PHL_RATING_THRESHOLDS.ABOVE_AVERAGE)
      return RATING_COLORS.NUMBER.ABOVE_AVERAGE;
    if (rating >= PHL_RATING_THRESHOLDS.AVERAGE)
      return RATING_COLORS.NUMBER.AVERAGE;
  }
...
export const getRatingBgColor = (rating: number | string): string => {
  if (typeof rating === "string") {
    return (
      RATING_BG_COLORS.STRING[rating as keyof typeof RATING_BG_COLORS.STRING] ||
      RATING_BG_COLORS.STRING.F
    );
  }

  if (rating >= NFL_RATING_THRESHOLDS.ELITE)
    return RATING_BG_COLORS.NUMBER.ELITE;
  if (rating >= NFL_RATING_THRESHOLDS.EXCELLENT)
    return RATING_BG_COLORS.NUMBER.EXCELLENT;
  if (rating >= NFL_RATING_THRESHOLDS.GOOD) return RATING_BG_COLORS.NUMBER.GOOD;
  if (rating >= NFL_RATING_THRESHOLDS.ABOVE_AVERAGE)
    return RATING_BG_COLORS.NUMBER.ABOVE_AVERAGE;
  if (rating >= NFL_RATING_THRESHOLDS.AVERAGE)
    return RATING_BG_COLORS.NUMBER.AVERAGE;
...
export const getAttributeColor = (value: any): string => {
  ...
  if (typeof value === "string") {
    return (
      ATTRIBUTE_COLORS.STRING[value as keyof typeof ATTRIBUTE_COLORS.STRING] ||
      ATTRIBUTE_COLORS.STRING.DEFAULT
    );
  }

  if (typeof value === "number") {
    if (value >= ATTRIBUTE_COLORS.NUMBER.EXCELLENT.threshold)
      return ATTRIBUTE_COLORS.NUMBER.EXCELLENT.color;
    if (value >= ATTRIBUTE_COLORS.NUMBER.GOOD.threshold)
      return ATTRIBUTE_COLORS.NUMBER.GOOD.color;
    if (value >= ATTRIBUTE_COLORS.NUMBER.AVERAGE.threshold)
      return ATTRIBUTE_COLORS.NUMBER.AVERAGE.color;
    if (value >= ATTRIBUTE_COLORS.NUMBER.BELOW_AVERAGE.threshold)
      return ATTRIBUTE_COLORS.NUMBER.BELOW_AVERAGE.color;
...
export const getGradeColor = (grade: string): string => {
  return (
    ATTRIBUTE_COLORS.STRING[grade as keyof typeof ATTRIBUTE_COLORS.STRING] ||
    ATTRIBUTE_COLORS.STRING.DEFAULT
  );
};
...
export const getAggressionLevelText = (
  percentage: number,
  type: "short" | "long",
): string => {
  const thresholds =
    AGGRESSION_THRESHOLDS[
      type.toUpperCase() as keyof typeof AGGRESSION_THRESHOLDS
    ];
...
export const getAggressionLevelColor = (
  percentage: number,
  type: "short" | "long",
): string => {
  const thresholds =
...
export const getPlayTypeColor = (
  playType: keyof typeof PLAY_TYPE_COLORS,
): string => {
  return PLAY_TYPE_COLORS[playType] || PLAY_TYPE_COLORS.default;
};

export const getFormationWeightColor = (weight: number): string => {
  if (weight > FORMATION_WEIGHT_COLORS.HIGH.threshold)
    return FORMATION_WEIGHT_COLORS.HIGH.color;
  if (weight > FORMATION_WEIGHT_COLORS.ACTIVE.threshold)
    return FORMATION_WEIGHT_COLORS.ACTIVE.color;
...
export const getPercentageStatusColor = (
  ...
  return total === target
    ? PERCENTAGE_STATUS_COLORS.COMPLETE
    : PERCENTAGE_STATUS_COLORS.INCOMPLETE;
};

export const getPercentageStatusBgColor = (
  total: number,
  target: number = 100,
): string => {
  return total === target
    ? PERCENTAGE_STATUS_BG_COLORS.COMPLETE
    : PERCENTAGE_STATUS_BG_COLORS.INCOMPLETE;
};
