...
import { getPriorityCBBAttributes, getPriorityNBAAttributes } from "../../Team/TeamPageUtils";
...
interface BasketballDepthChartProps {
  ...
  SwapLineupPlayers: (index: number, firstKey: string, secondKey: string) => void;
}

const displayRating = (player: BasketballPlayer, value: number, league: League) => league === SimCBB ? getCBBLetterGrade(value, player.Year) : value;
const overallBadgeClass = (overall: number | string, player: BasketballPlayer, league: League) => {
...
const overallTextClass = (overall: number | string, player: BasketballPlayer, league: League) => {
...
const eligibleForPosition = (player: BasketballPlayer, position: string) =>
  position === "G" ? player.Position === "G" || player.Position === "F" : position === "C" ? player.Position === "C" || player.Position === "F" : true;

export const BasketballDepthChart: FC<BasketballDepthChartProps> = ({ selectedPositionIndex, lineupFormation, selectedTeamLineups, selectedRosterMap, selectedTeamRoster, team, league, canModify, ChangeLineupInput, SwapLineupPlayers }) => {
  ...
  const availablePlayers = useMemo(() => selectedTeamRoster.filter((player) => {
    if (!eligibleForPosition(player, position)) return false;
    const usedInFirstElsewhere = targetKey === "FirstStringID" && selectedTeamLineups.some(
      (other, index) => index !== selectedPositionIndex && other.FirstStringID === player.ID,
    );
    const alreadyAssignedToThisPosition = lineup && stringSlots.some(
      (slot) => lineup[slot.key] === player.ID,
    );
    return !usedInFirstElsewhere && !alreadyAssignedToThisPosition;
  }), [lineup, position, selectedPositionIndex, selectedTeamLineups, selectedTeamRoster, targetKey]);
  ...
  const selectAvailablePlayer = (player: BasketballPlayer) => {
    ...
    ChangeLineupInput(lineup[targetKey], targetKey, player.ID, selectedPositionIndex);
    setTargetKey(null);
  };
  ...
  return <>
    ...
    <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={`Manage ${slotLabel} Depth Chart`} maxWidth="max-w-6xl">
      <div className="mb-4 flex items-center justify-center gap-3 rounded-lg bg-slate-800/70 p-3"><Text variant="body" classes="font-semibold">View:</Text><ButtonGroup><Button size="xs" isSelected={!showAttributes} onClick={() => setShowAttributes(false)}>Player Cards</Button><Button size="xs" isSelected={showAttributes} onClick={() => setShowAttributes(true)}>Attributes</Button></ButtonGroup></div>
      <div className="max-h-[70vh] space-y-6 overflow-y-auto">
        <div><Text variant="h5" classes="mb-3 font-semibold">Current Position Levels ↕</Text><div className="mb-3 rounded-lg bg-[#23439b] p-3 text-center text-sm text-slate-200">{targetKey ? `${slotLabel} ${stringSlots.find((slot) => slot.key === targetKey)?.label} selected. Choose another level to swap, or choose an eligible player.` : "Select a position level or an available player to begin swapping."}</div><div className={`grid gap-3 ${showAttributes ? "grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5" : "grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6"}`}>{stringSlots.map((slot) => <ModalPlayerCard key={slot.key} player={selectedRosterMap[lineup[slot.key]]} level={`${slotLabel}-${slot.label}S`} selected={targetKey === slot.key} showAttributes={showAttributes} team={team} league={league} onClick={() => selectSlot(slot.key)} onRemove={canModify && slot.key === "ThirdStringID" && lineup[slot.key] ? () => { ChangeLineupInput(lineup[slot.key], slot.key, 0, selectedPositionIndex); setTargetKey(null); } : undefined} />)}</div></div>
        <div><Text variant="h5" classes="mb-3 font-semibold">Available Players</Text><div className={`grid gap-3 ${showAttributes ? "grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5" : "grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6"}`}>{availablePlayers.map((player) => <ModalPlayerCard key={player.ID} player={player} showAttributes={showAttributes} team={team} league={league} onClick={() => selectAvailablePlayer(player)} />)}</div>{!availablePlayers.length && <Text variant="body" classes="py-8 text-center text-slate-400">No available players for this position.</Text>}</div>
      </div>
    </Modal>
  </>;
};
...
const PlayerCard: FC<{ player: BasketballPlayer; team: any; league: League }> = ({ player, team, league }) => {
  const overall = displayRating(player, player.Overall, league);
  const primary = team?.ColorOne || "#1e3a8a";
  const accent = team?.ColorThree || "#ffffff";
  const textColor = getTextColorBasedOnBg(primary);
  return <div className="relative h-40 overflow-hidden rounded-lg border-2 border-black p-2 text-center" style={{ backgroundColor: primary, backgroundSize: "4px 4px" }}><span className="absolute left-0 top-0 rounded-br-md bg-black/80 px-1 py-0.5 text-xs font-bold text-white">{player.Position}</span><span className={`absolute right-0 top-0 rounded-bl-md bg-black/80 px-1 py-0.5 text-xs font-bold ${overallTextClass(overall, player, league)}`}>{overall}</span><div className="relative z-10 flex h-full flex-col justify-between"><div className="mx-auto mt-2 flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-white [&_svg]:h-full [&_svg]:w-full"><PlayerPicture playerID={player.ID} player={player} team={team} league={league} classes="h-full w-full" /></div><div className="px-1"><strong className={`block truncate text-xs ${textColor}`} style={{ textShadow: textColor.includes("white") ? "1px 1px 0 black, -1px -1px 0 black, 1px -1px 0 black, -1px 1px 0 black" : "1px 1px 0 white, -1px -1px 0 white, 1px -1px 0 white, -1px 1px 0 white" }}>{player.FirstName}</strong><strong className={`block truncate text-xs ${textColor}`} style={{ textShadow: textColor.includes("white") ? "1px 1px 0 black, -1px -1px 0 black, 1px -1px 0 black, -1px 1px 0 black" : "1px 1px 0 white, -1px -1px 0 white, 1px -1px 0 white, -1px 1px 0 white" }}>{player.LastName}</strong><span className={`mt-1 block rounded-lg bg-black/55 px-1 py-0.5 text-xs font-semibold ${textColor}`}>{player.Archetype}</span></div></div></div>;
};

const AttributeCard: FC<{ player: BasketballPlayer; team: any; league: League }> = ({ player, team, league }) => {
  const attributes = league === SimCBB ? getPriorityCBBAttributes(player as CollegePlayer) : getPriorityNBAAttributes(player as NBAPlayer);
  const primary = team?.ColorOne || "#1e3a8a";
  const textColor = getTextColorBasedOnBg(primary);
  const overall = displayRating(player, player.Overall, league);
  const abbreviations: Record<string, string> = { Agility: "AGI", "Inside Shooting": "INS", "Mid Range Shooting": "MID", "MidRange Shooting": "MID", "3pt Shooting": "3PT", "Free Throw": "FT", Ballwork: "BH", Stealing: "STL", Rebounding: "REB", Blocking: "BLK", "Int. Defense": "INT D", "Per. Defense": "PER D" };
  return <div className="relative h-52 overflow-hidden rounded-lg border-2 border-black p-2" style={{ backgroundColor: primary }}><span className="absolute left-1 top-1 rounded-sm bg-black/85 px-1.5 py-0.5 text-xs font-bold text-white">{player.Position}</span><span className={`absolute right-1 top-1 rounded-sm bg-black/85 px-1.5 py-0.5 text-xs font-bold ${overallTextClass(overall, player, league)}`}>{overall}</span><div className={`absolute left-10 right-10 top-1 text-center text-xs ${textColor}`} style={{ textShadow: textColor.includes("white") ? "1px 1px 0 black, -1px -1px 0 black" : "1px 1px 0 white, -1px -1px 0 white" }}><strong className="block truncate">{player.FirstName}</strong><strong className="block truncate">{player.LastName}</strong></div><div className="mt-10 grid grid-cols-4 gap-1 text-center text-[10px]">{attributes.slice(0, 11).map((attribute) => <span key={attribute.label} className="rounded-sm bg-black/70 p-1"><span className="block text-[9px] font-semibold text-slate-100">{abbreviations[attribute.label] || attribute.label}</span><strong className={`text-xs ${attributeTextClass(attribute.value, league)}`} style={{ WebkitTextStroke: "0.3px black", textShadow: "0 1px 1px black" }}>{attribute.value}</strong></span>)}</div></div>;
};
